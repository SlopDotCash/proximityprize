\section{The Subfield \texorpdfstring{$R_q^H \cong \F_{q^k}$}{R q fixed by H, isomorphic to F q to k} (Hachi Lemma 5)}\label{sec:hachi_subfield}
...
\subsection{Reduction to ``\texorpdfstring{$R_q^H$}{R q fixed by H} is a field''}
...
\subsection{The crux: \texorpdfstring{$R_q^{\sigma_{-1}}$}{R q fixed by sigma minus one} is a field}
...
\subsection{The swap (Phase 4): \texorpdfstring{$\sigma_{-1}$}{sigma minus one} and polynomial reversal}
...
\texttt{Polynomial.eval₂\_reverse\_mul\_pow} at the point $(\mathrm{mk}\,X)^{-1} = (\mathrm{mk}\,X)^{c}$.
\end{lemma}
