...
The most likely failure mode is isolated by the API itself.  A single-witness bad scalar has
`R = 1`, so the multiplicity discount buys nothing.  A positive use must prove that the hard
large-zero safe lines cannot have sparse scalar witnesses, probably from stack geometry or a
separate ownership law.
