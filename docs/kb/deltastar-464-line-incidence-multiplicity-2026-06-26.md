...
The fallback can also be attacked per witness codeword.  The new codeword-singleton route replaces
the total singleton defect by a uniform cap on
`codewordSingletonWitnessScalars` and the actual appearing-codeword count:

```text
punctured weight + #appearingCodewords * perCodewordSingletonCap <= 2B.
```

There is also a coarser line-list form using a large-zero-safe line-list cap `Lzero`.  The matching
scanners now have exact failure forms and combined dichotomies: failed production exposes either
combined arithmetic failure, line-list-cap failure, or a concrete appearing codeword whose
singleton-witness scalar fiber exceeds the proposed cap.
That concrete cap failure is now tied back to the old denominator wall:
`codewordSingletonWitnessScalars` is contained in the codeword's heavy-scalar set, so on a
zero-safe line it is at most `support(u1)/(a - #zeroAgreement(c))`.  Thus any failed uniform
per-codeword cap already produces a codeword whose ordinary support-denominator capacity exceeds
the cap.
