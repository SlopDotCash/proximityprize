...
The remaining formal step is to sum the now-classified literal profile fibers and transport
the resulting canonical signed-tuple count through `signedIndexEquiv` to `NR`.  The weld to
`NR` is therefore not complete, and (E) is not yet claimed as a Lean theorem.
