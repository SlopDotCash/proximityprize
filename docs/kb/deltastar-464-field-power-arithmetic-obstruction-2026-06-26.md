...
overfull finite cover sum must occur at `t < k`.  The low/high split is now also a positive
production interface: prove the finite cover-sum budget only for `t < k`, pay high profiles by the
scalar-times-support-binomial ceiling, and
`uniformLineBadScalarsBudgeted_of_lowSupportRatioCoverSums` recovers the bad-scalar budget.  The
direct converse
`exists_largeZero_safe_low_supportRatioCoverSum_gt_of_not_uniformLineBadScalarsBudgeted` exposes an
overfull large-zero safe low cover sum whenever those fixed side hypotheses still fail to produce
the budget.  The same positive/converse pattern is named one layer earlier for
support-ratio-heavy coordinate fibers:
`uniformLineBadScalarsBudgeted_of_lowSupportRatioHeavyCoordFibers` uses the high-profile singleton
ceiling, and `exists_largeZero_safe_low_supportRatioHeavyCoordFiber_gt_of_not_budgeted` extracts
the corresponding low heavy-fiber overflow.
