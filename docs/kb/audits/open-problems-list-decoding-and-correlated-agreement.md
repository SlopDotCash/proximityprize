...
| Lemma 3.7 Elias lower bound | present | `lambda_ge_elias_volume_eli57` (generic) + `linear_lambda_ge_elias_volume_eli57` (field specialization) in [Bounds/General.lean](../../../ArkLib/Data/CodingTheory/ListDecoding/Bounds/General.lean), helpers `card_filter_hammingDist_le_eq_hammingBallVolume` / `sum_ncard_closeCodewordsRel_eq_of_set` | **PROVEN in-tree, sorry-free and axiom-clean**, by [Eli57] averaging: mean point-list size over random centre is `|C|·Vol/q^n = Vol/q^{n-k}`, so some centre attains at least the mean. Generic over any finite alphabet with `|C| = q^k`; field-linear is one-line specialization. |
| Corollary 3.8 volume-based lower bound | present | `lambda_ge_entropy_volume` (generic) + `linear_lambda_ge_entropy_volume` (field specialization) + `qary_shell_entropy_lower` in [Bounds/General.lean](../../../ArkLib/Data/CodingTheory/ListDecoding/Bounds/General.lean) | **PROVEN in-tree, sorry-free and axiom-clean** via MS77 Stirling bound (`ABF26C38.ms77_lattice` at lattice points) + generic Elias core. Generic over any nontrivial finite alphabet; field-linear is specialization. |
