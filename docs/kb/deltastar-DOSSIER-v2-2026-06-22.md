...
6. **There is exactly one genuinely off-BGK obstruction-removal lever still standing** (§9): the **bad-prime localization / least-prime-in-AP** route for the explicit binder-family floor obstruction. It is a 0-dimensional cyclotomic-resultant question, *not* a character sum. The exact closure premise is: if "floor-bad primes = the single smallest prime ≡ 1 mod n" holds uniformly in `μ`, and that least prime is below prize scale `n^4`, then every prize-scale prime is good for this modeled binder predicate. Classical exponent-5 Linnik does **not** by itself imply the below-`n^4` premise; the route needs a sub-4 AP-prime input such as the in-tree Thorner--Zaman supply bridge (`β ≤ 3`), GRH/Montgomery-strength input, or a dyadic-special least-prime theorem. Proven at `n = 16` (exhaustive). This is still a useful arithmetic check, but §16 is the governing correction: binder-family floor-goodness is necessary, not sufficient, for the δ\* lower pin.
