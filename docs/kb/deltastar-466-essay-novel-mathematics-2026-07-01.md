...
| id | statement (informal) | attack |
|---|---|---|
| CMK | abstract moment-problem: equal-atom + Parseval + `K^r`-Wick-to-depth-`2log q` ⟹ `M² ≤ C(K)·n·log q` via Christoffel crowding | prove/refute as pure analysis; P4 measures ingredients |
| SST | all-but-`K^r`-many `2r`-sections of `L_𝔭` have random-size dual minima; dilation orbit structure on supports compresses the union | probe dual minima at small n; then transference bookkeeping |
| γ₂-degeneration | on flat-covariance exchangeable families, γ₂ = union bound exactly | provable; gate brick |
| Vertical-MSS death | no interlacing + bad-prime-dominated expectation | gate brick `_VerticalMSSGate` |
| TPS | typical-prime sieve boundary at `r ≈ β` (provable); divisor-equidistribution beyond (open) | boundary: land as brick; beyond: named Prop |
| CandidateListExactSuccessor, low-profile `D(t)`, windowed SumsetExtremal, Hankel short-window `k*` predictor | dossier Tier-1 carried forward | P4/P5 + Lean lanes |

**What would actually close the prize, in one sentence per route:** (A) prove CMK *and* the
