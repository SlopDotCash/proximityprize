...
| NWS-8 transition/intersection arithmetic | **EXACT C12 ROW COUPLING / marginal NO-GO; alignment active** | `C12=Σ_tW_G(t)R_r(t)` with exact masses `n²` and `C(n,r)C(n,r−1)`. The centered numerator `A_r=qC12−n²C(n,r)C(n,r−1)` is the literal gate target; mean products have 319/374 bits. A two-cell theorem makes the inner product range from 0 to full under identical marginal L2 data, so joint placement is indispensable. |
