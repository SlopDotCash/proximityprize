...
| ESM-3 entropic CLT | **ROBUST LATER-STEP WICK SOCKET / five transitions open** | One full-unit improvement or the distributed final profile `(10.5,12.5)` closes even after multiplying all six bounds by `501/500` (distributed scaled product `125527.08...<126871`). Antipodal pairs put the actual first ratio above `3+2^-29`, so the saving must occur at transitions 2→3 through 6→7. |
