...
| ESM-3 entropic CLT | **EXACT LATER DEFECT LEDGERS / five transitions open** | Rational caps are exactly `Bn(r+1)^2Δ_(r+1)≤A(n-r)^2Δ_r`. One full-unit improvement or the distributed late pair `(10.5,12.5)` closes under `501/500`; its obligations have coefficients `10521` and `12525`. The first ratio is above `3+2^-29`, so only transitions 2→3 through 6→7 remain. |
...
| ESM-6 exact trajectory cells | **PROBE only / density theorem refuted** | Selected-cap successes rank 6→7 first and 5→6 second; favorable order-64 cells satisfy the distributed tail. But `(64,8001281)` has `C(64,5)<p< C(64,6)` and still `c5=11.073...`, `c6=13.137...`, above Wick. Birthday crossing alone is false; no finite cell proves production. |
...
| ESM-10 entropy-production inverse theorem | **SIGNED CORRELATION SOCKET / later defect open** | At 2→3 the exact cubic census is `C111,111+9C12,12+4n−6C111,12+4C111,3−12C12,3`; the robust defect is iff a lower bound on `6C111,12+12C12,3`. G186 Young controls the positive weighted square but drops exactly this favorable mass. At 5→6/6→7 use the explicit `10521/12525` ledgers. |
...
1. **PPCT-8 injective capstone:** the repeated cone, including its `1/1024` noncircular
   ...
   the primitive Wick ray leaves exactly `8264`, or `6.115...%`, to save arithmetically.
