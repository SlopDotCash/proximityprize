...
| NWS-8 transition/intersection arithmetic | **EXACT PHYSICAL / FOURIER / ORBIT COORDINATES; sign and rearrangement REFUTED; relative gate active** | `C12=Σ_tW_G(t)R_r(t)` and `A_r=qC12−n²C(n,r)C(n,r−1)` are exact. Fourier gives `A_r=Σ_(b≠0)η(2b)conjη(b)conj(e_r(b))e_(r−1)(b)`; orbit compression proves `C12` and `A_r` are divisible by `n`. Nonzero `W` has a published production-applicable cyclotomic half cap, but formal magnitude/half-cap models still have opposite or zero/full alignment. In 38 exact cells, `A_r<0` in 19, all sign/gate quadrants occur, and comonotonicity fails in 36. The live theorem is the relative `A_r≥A_required(C11,C22,Δ_r)`. |
...
| ESM-5 collision-entropy tensorization | **GENERIC DISTINCT-SAMPLING / UNPOINTED JOHNSON ROUTES REFUTED** | Coupling misses by over `141` energy bits. Mark-forgetting on a pointed subset fibre is rank one with a zero-sum kernel; `(1,−1)` rules out every finite pointed-energy comparison, including `10.5/12.5`. Fresh weight3 retains a `2x` twist, so only a pointed, colour-sensitive subgroup theorem survives. |
