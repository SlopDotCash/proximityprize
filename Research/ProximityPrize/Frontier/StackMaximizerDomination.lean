...
/-- Existence of a bounded dominating stack is exactly the full worst-case incidence budget.
The forward direction consumes the candidate certificate; the reverse direction chooses a genuine
global maximizer and bounds it by the universal budget. -/
theorem exists_singleStackDominationCertificate_iff_worstCaseIncidenceBounded
    (C : Set (ι -> A)) (δ : ℝ≥0) (B : ℕ) :
    (∃ uMax : WordStack A (Fin 2) ι,
      StackDominates F C δ uMax ∧ StackBounded F C δ uMax B)
      ↔ ProximityGap.OpenCoreConditionalPin.WorstCaseIncidenceBounded
          (F := F) (A := A) C δ B := by
  constructor
  · rintro ⟨uMax, hmax, hbounded⟩
    exact worstCaseIncidenceBounded_of_stackDominates C δ hmax hbounded
  · intro hI
    rcases exists_stackDominates (F := F) (A := A) C δ with ⟨uMax, hmax⟩
    exact ⟨uMax, hmax, stackBounded_of_worstCaseIncidenceBounded C δ hI uMax⟩
...
/-- Failure of every bounded single-stack domination certificate is exactly an above-budget stack.
Since a dominating stack always exists nonconstructively, the only obstruction to such a
certificate is the actual worst-case count exceeding the proposed budget. -/
theorem not_exists_singleStackDominationCertificate_iff_exists_budget_lt_stackBadCount
    (C : Set (ι -> A)) (δ : ℝ≥0) (B : ℕ) :
    (¬ ∃ uMax : WordStack A (Fin 2) ι,
      StackDominates F C δ uMax ∧ StackBounded F C δ uMax B)
      ↔ ∃ u : WordStack A (Fin 2) ι, B < StackBadCount F C δ u := by
  rw [exists_singleStackDominationCertificate_iff_worstCaseIncidenceBounded]
  exact not_worstCaseIncidenceBounded_iff_exists_budget_lt_stackBadCount C δ B
...
/-! ## Axiom audit -/
#print axioms not_stackBounded_iff_budget_lt_stackBadCount
#print axioms exists_stackDominates
#print axioms worstCaseIncidenceBounded_of_stackDominates
#print axioms stackBounded_of_worstCaseIncidenceBounded
#print axioms worstCaseIncidenceBounded_iff_stackBounded_of_stackDominates
#print axioms exists_stackDominates_and_worstCaseIncidenceBounded_iff_stackBounded
#print axioms exists_singleStackDominationCertificate_iff_worstCaseIncidenceBounded
#print axioms deltaStar_pin_of_stackMaximizer
#print axioms not_stackDominates_of_exists_strictly_larger
#print axioms not_worstCaseIncidenceBounded_of_budget_lt_stackBadCount
#print axioms not_worstCaseIncidenceBounded_iff_exists_budget_lt_stackBadCount
#print axioms not_exists_singleStackDominationCertificate_iff_exists_budget_lt_stackBadCount
#print axioms candidateBounded_and_counterStack_not_worstCaseIncidenceBounded
#print axioms candidateBounded_not_dominationProof_of_strictly_larger
