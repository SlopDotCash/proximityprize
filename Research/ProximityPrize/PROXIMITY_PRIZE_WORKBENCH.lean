...
-/
import Mathlib
import Research.ProximityPrize.All
...
open ArkLib.ProximityGap.SubgroupGaussSumFourthMoment
  (addEnergy subgroup_gaussSum_fourthMoment)
...
/-! ## §4  Entropy-based candidate expression

Equating a heuristic average line incidence with a chosen budget suggests an
inverse-entropy expression. This section defines that expression; it does not
prove an incidence estimate, an upper bound, or an equality for production δ*. -/

/-- Lower generalized inverse of normalized q-ary entropy on `[0, 1-1/q]`.
The infimum gives a total definition, including the empty-set case. Inverse identities
require `1 < q` and a target in `[0,1]`; none are asserted by this definition. -/
noncomputable def qaryEntropyInv (q : ℕ) (y : ℝ) : ℝ :=
  sInf {x : ℝ | 0 ≤ x ∧ x ≤ 1 - 1 / (q : ℝ) ∧
    y ≤ Real.qaryEntropy q x / Real.log (q : ℝ)}

/-- Candidate average-incidence threshold `H_q⁻¹(1 − ρ − log_q(1/εStar)/n)`.
No theorem here identifies this expression with the worst-case proximity threshold.
Even inverse identities need the parameter and range conditions described above. -/
