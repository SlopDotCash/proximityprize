...
/-!
# Hypothesis K4 wired: the assembler re-run against the radical (#304)

**The `hRsep` audit** (every use of `hRsep`/`hR : R.Separable` through
`Section5GlobalAssembler` / `SectionGlobalLift` / `DecodedRootSupply`):

1. **The `(hsplit, hbr)` production** (`SectionGlobalLift.split_canonical` /
   `hbr_canonical`) does NOT consume the external `hRsep` at all: it consumes the
   **fiber** separability `b.hHyp.separable_evalX` (a `Hypotheses` structure field).
   THIS use is deleted by the radical: `split_canonical_radical` + `hbr_canonical_radical`
   below produce the canonical package at `radical (evalX (C x₀) b.R)` with squarefreeness
   FREE OF CHARGE (`squarefree_radical`); the only residue of `separable_evalX` consumed is
   its *nonzeroness* consequence (`fiber_ne_zero` — `Separable.ne_zero`, zero squarefreeness
   content).

2. **The single genuine consumption of the external `hRsep : b.R.Separable`** in the
   front-door chain is `DecodedRootSupply.mpFin_of_decoded_roots`'s `hR`, which bottoms out
   at `specialized_separable_of_R_separable` (`MatchingPointFromLocalSeries.lean`): the
   image separability of the doubly-mapped matching polynomial over `PowerSeries F` for the
   per-place Hensel matching point.  This is NOT a fiber-squarefreeness use, and the radical
   CANNOT discharge it: it constrains `R` itself — the object whose Hensel numerator
   `βHensel` is being analyzed — and substituting `radical R` would change `γ`/`βHensel`
   (a different theorem, not a hypothesis weakening).  The honest available handling is the
   landed weakening `MappedSeparability.MappedSliceSeparability` (strictly below `hRsep`,
   producible from per-place field-level residue data) — which is what the radical front
   door below consumes.

**Net front-door effect** (`hcoeffPoly_witness_of_producersOn_radical` vs the landed
`hcoeffPoly_witness_of_producersOn_branch`): the global structural surface stays
`hdvdR` (+ `hwdeg`) + `hOnH`, the branch certificate moves to the radical cofactor
`radical (evalX (C x₀) b.R) /ₘ b.H` (no squarefreeness hypothesis consumed anywhere in its
production), and `hRsep` is REPLACED by the strictly weaker `MappedSliceSeparability` —
full deletion is impossible per item 2 (`..._radical_sep` restores the `hRsep` drop-in
shape via `MappedSliceSeparability.of_separable`).

The radical decoded-root supply (`rootDecodedRadical` & co.) mirrors
`DecodedRootSupply.rootDecoded` with the GS split replaced by the radical split: the
centre-fold linear factor is prime, hence divides the radical of the fiber, so every place
of the radical branch-certificate locus decodes a rational root of `H` — squarefreeness of
the fiber never enters.

**The degree-budget transfers** (`natDegree_radical_le` through the `gradedCardBudget`
inequalities): degree additivity of the canonical radical split
(`natDegree_radical_fiber_eq_add`) bounds the radical cofactor by
`natDegree(fiber) − natDegree(H)` (`natDegree_radical_cofactor_le`); the bivariate
eval-degree bound (`eval_natDegree_le_of_coeff_le`) then caps the radical branch
certificate's degree (`cert_radical_natDegree_le`), discharging the exact `hbig` field of
the radical assembler from a numeric budget (`hbig_radical_of_coeff_budget`).  In the
overlap regime (fiber certificate nonzero — where the non-radical route is available at
all), the radical certificate *divides* the fiber certificate
(`radical_cofactor_dvd_cofactor` / `cert_radical_dvd_cert_fiber`), so the assembler's
original `hbig` transfers monotonically (`hbig_radical_of_hbig_fiber`): the radical route
never worsens the §6 counting.
-/
...
#print axioms ArkLib.RadicalWire304.fiber_ne_zero
#print axioms ArkLib.RadicalWire304.H_dvd_radical_fiber
#print axioms ArkLib.RadicalWire304.natDegree_radical_fiber_le
#print axioms ArkLib.RadicalWire304.radical_fiber_ne_zero
#print axioms ArkLib.RadicalWire304.eval_natDegree_le_of_coeff_le
#print axioms ArkLib.RadicalWire304.split_canonical_radical
#print axioms ArkLib.RadicalWire304.hbr_canonical_radical
#print axioms ArkLib.RadicalWire304.split_branch_radical
#print axioms ArkLib.RadicalWire304.natDegree_radical_fiber_eq_add
#print axioms ArkLib.RadicalWire304.natDegree_radical_cofactor_le
#print axioms ArkLib.RadicalWire304.cert_radical_natDegree_le
#print axioms ArkLib.RadicalWire304.hbig_radical_of_coeff_budget
#print axioms ArkLib.RadicalWire304.radical_cofactor_dvd_cofactor
#print axioms ArkLib.RadicalWire304.cert_radical_dvd_cert_fiber
#print axioms ArkLib.RadicalWire304.hbig_radical_of_hbig_fiber
#print axioms ArkLib.RadicalWire304.centreFold_dvd_radical
#print axioms ArkLib.RadicalWire304.evalEval_radical_eq_zero
#print axioms ArkLib.RadicalWire304.rootDecodedRadical
#print axioms ArkLib.RadicalWire304.rootDecodedRadical_val
#print axioms ArkLib.RadicalWire304.rootDecodedRadical_val_monic
#print axioms ArkLib.RadicalWire304.H_eval_centreFold_eq_zero_radical
#print axioms ArkLib.RadicalWire304.xiCert_eq_derivativeCert_of_centreFold_root
#print axioms ArkLib.RadicalWire304.xiCert_isUnit_of_centreFold_root
#print axioms ArkLib.RadicalWire304.xiCert_eval_monic_radical
#print axioms ArkLib.RadicalWire304.hx_of_global_structural_radical
#print axioms ArkLib.RadicalWire304.mpFin_of_decoded_roots_radical_mapped
#print axioms ArkLib.RadicalWire304.mpFin_of_decoded_roots_radical
