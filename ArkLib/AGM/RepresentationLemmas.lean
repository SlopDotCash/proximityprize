...
* `inv_target_mem_closure` / `inv_target_mem_of_forall_mem` — inverse soundness: the inverse of a
  represented target stays in the subgroup generated by (resp. containing) the basis.
-/
...
/-- **Inverse soundness.** The inverse of a represented target lies in the same subgroup generated
by the basis (subgroups are closed under inversion). The soundness complement to
`GroupRepresentation.inv`: like `inv`, no exponent-length hypothesis is needed because this is a
pure membership fact rather than the concrete inverse construction. -/
theorem inv_target_mem_closure {prev : List G} {target : G}
    (repr : GroupRepresentation (p := p) prev target) :
    target⁻¹ ∈ Subgroup.closure {g | g ∈ prev} :=
  Subgroup.inv_mem _ repr.target_mem_closure

/-- **Subgroup-relative inverse soundness.** If every basis element lies in a subgroup `H`, the
inverse of the represented target lies in `H`. -/
theorem inv_target_mem_of_forall_mem {prev : List G} {target : G}
    (repr : GroupRepresentation (p := p) prev target) (H : Subgroup G)
    (hprev : ∀ g ∈ prev, g ∈ H) : target⁻¹ ∈ H :=
  Subgroup.inv_mem _ (repr.target_mem_of_forall_mem H hprev)
...
#print axioms AGM.GroupRepresentation.inv_target_mem_closure
#print axioms AGM.GroupRepresentation.inv_target_mem_of_forall_mem
