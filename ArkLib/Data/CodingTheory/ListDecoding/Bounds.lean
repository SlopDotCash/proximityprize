...
- `lambda_ge_elias_volume_eli57` / `linear_lambda_ge_elias_volume_eli57` — ABF26 L3.7 [Eli57]: `|Λ(C, δ)| ≥ Vol_q(δ, n) / q^{n-k}` (generic core + field specialization).
- `lambda_ge_entropy_volume` / `linear_lambda_ge_entropy_volume` — ABF26 C3.8: `|Λ(C, δ)| ≥ q^{n(ρ-1+H_q(δ))} / √(8nδ(1-δ))` (generic core + field specialization).
...
- `lambda_ge_elias_volume_eli57` (L3.7 [Eli57]) — Elias volume list-size lower bound (generic core over any finite alphabet with `|C| = q^k`, plus field-linear specialization `linear_lambda_ge_elias_volume_eli57`).
- `lambda_ge_entropy_volume` (C3.8) — entropy-volume lower bound (generic core over any nontrivial finite alphabet, plus field specialization `linear_lambda_ge_entropy_volume`; MS77 Hamming-ball volume via Robbins–Stirling, all in-tree).
