    ...
    · -- sInf S_dists ≤ sInf S (because sInf S_dists is a lower bound for S)
      ...
      · intro d' hd'
        ...
        have h_sInf_le_fv := csInf_le (by
          use 0; intro; (expose_names; exact fun _ ↦ bot_le)) (Set.mem_image_of_mem f hv_mem)
...
lemma relDist_floor_bound_iff_complement_bound (n upperBound : ℕ) (δ : ℝ≥0) :
  ...
  set r : ℝ≥0 := δ * n
  set m : ℕ := Nat.floor r
  have h_m_le_r_NNReal : (m : NNReal) ≤ r := Nat.floor_le (a := r) (ha := bot_le)
