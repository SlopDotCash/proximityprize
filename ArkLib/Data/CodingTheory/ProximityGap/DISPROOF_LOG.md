...
## 2026-06-14 (#407) — four shortcuts REFUTED by the 9-agent feasible-grind workflow (2 reps each)

All from real probe runs (scripts/probes/_wf_*.py), independent replicates cross-validated.

1. **REFUTED: "universal house constant C = √2 (or 1.33)."** Exact data: C climbs with n (1.06→1.33,
   n=8..64, β=4), is β-dependent (decreases with β under ln(q/n); flat under ln q), and one exact point
   C=1.487 > √2. No clean universal constant; sharp form is `B ≲ C·√(n·ln q)`, C≈1.0–1.1 typical. The
   prior RESULTS-407 §6 "flat plateau 1.16–1.33" is corrected (see §7 there).
2. **REFUTED: "the moment arrow closes B ≤ √(n log q) at constant index."** E_r IS random-like
   (settled YES, C≈1–1.6 bounded in n and r, c_r decreasing in r — the E_3-spike debate was
   prime-resonance), but at constant index the arrow min_r (p·E_r)^{1/2r} saturates at the trivial n
   (arrow/trueB diverges 2.1→11.4). Random-likeness necessary, not sufficient; needs max|S(b)| directly
   = the wall.
3. **REFUTED: "the δ* upper-half floor = one closed form keyed to the monomial ladder N_fib."** The
   ladder is NOT list-maximal: nodal Laurent words `x^{-1}+x^b` beat N_fib above Johnson (7>3, 35>7,
   115>28), verified across ≥3 sparse primes + independent brute force. Correct floor =
   `max(N_fib, L_nodal)`; nodal wins at the deep end (near capacity). (Consistent with the in-tree
   sub-Johnson `NodalSupplyGeneralK`; now confirmed ABOVE Johnson too.)
4. **REFUTED (methodology): "no word beats the ladder" reads from weak search.** Random hill-climb and
   base+corruption enumeration systematically UNDER-find the nodal beaters (reported max 2 < N_fib,
   wrongly concluding "ladder holds"). Only ladder-neighborhood perturbation search found them. Prior
   "ladder holds / floor uniform" conclusions are false negatives unless they used neighborhood search.

**Net:** four wrong shortcuts removed (genuine progress); the δ* ansatz `H_q^{-1}((1-ρ)-log_q(1/ε)/n)`
survives as validated up-to-Johnson + correct curve shape/window, but its deep-band floor must be
`max(N_fib, L_nodal)` and the asymptotic beat-ratio + house-constant convergence both reduce to the
same open growing-n Gauss-period / deep-moment wall. No closure fabricated.

### FORMALIZED (2026-06-14): the ladder-not-floor refutation is now a machine-checked Lean brick
`ArkLib/Data/CodingTheory/ProximityGap/LadderFloorRefutation.lean` — `ladder_not_floor` (no axioms,
kernel `decide`): the nodal product-supply `C(n,r)/n` strictly exceeds the ladder antipodal fibre
`N_fib(2^{h+1},r)=C(2^h−1,(r−1)/2)` at n=8,16,32 and odd r (3<7, 7<35, 21<273, 15<155, 105<6293).
Plus `nodal_floor_gap_grows_concrete`: `3·nodalSupply(2^{h+1},3)=(2^{h+1}−1)(2^h−1)`, so the gap ratio
`(2^{h+1}−1)/3 ≈ 2^{h+1}/3` GROWS with tower height (not O(1) slack). Mechanism: the ladder only sees
±-paired (antipodal) subsets; the nodal word's product-pinning sees all of them. Confirms #3 above as
an in-tree theorem, not just a probe.

### GENERALIZED (2026-06-14): floor break proven at EVERY tower height (incl. prize n=2^32)
`LadderFloorRefutation.ladder_not_floor_general (h≥2)`: `ladderNfib h 3 < nodalSupplyValue (2^{h+1}) 3`
for ALL h — via the division-free core `nodal_choose_lower : a·(2a) ≤ C(2a,3)` for `a≥4` (from
`6·C(n,3)=n(n-1)(n-2)` [Nat.choose_succ_right_eq] + `6a≤(2a-1)(2a-2)`). So the ladder-only floor is
broken at every smooth-domain size n=2^{h+1}, not just probed n≤32. (Caveat: r=3 = deepest radius
δ→1; the prize-window large-r `r≈ρn` analogue stays the open deep-moment wall.) All 4 thms axiom-clean.

## #407 CHENG-HOUSE route — dyadic √2 house floor PROVEN (modest), but WRONG DIRECTION for the count (2026-06-13)

**Technique.** Cheng et al. (J.Number Theory 2022, "Solution counts and sums of roots of unity")
equate the p-defect count to LOWER bounds on the **house** (= max conjugate modulus) of sparse ±sums
of n-th roots of unity. The general worst-case house lower bound is exponentially small
(`house ≥ (n+1)^{-p}`, Habegger/Myerson) — useless. Question: does the **dyadic** `n=2^μ` structure
(antipodal `ζ^{n/2}=−1`, `i=ζ^{n/4}`, Lam–Leung tower) give a worst-case house **lower** bound
`≥ c > 0` strong enough to force a defect α (≢0) OUT of the prize box `‖α‖ ≤ √(n/2)·2r`?

**NEW PROVEN result (modest, axiom-clean-able).** For a nonzero **balanced** sparse ±sum
`α = (Σ r plus-roots) − (Σ r minus-roots)` of `2^μ`-th roots of unity, the **house is bounded below
by `√2`, uniformly in `n` and `r`, with a spectral GAP — no balanced house lies in `(1, √2)`.**
- Probes `probe_cheng_house_balanced_407.py`, `probe_cheng_house_floor_proof_407.py`: `min house =
  √2` exactly for `n=4..32`, `r=1,2,3`; smallest distinct houses are `√2, 1.848, 2, …` (gap at `√2`).
- **Provable mechanism (dyadic):** the extremal `α = ζ^a − ζ^{a+n/4}` (the `i`-rotated/orthogonal
  pair) has `σ_t(α) = ζ^{ta}(1 − ζ^{t·n/4})`; for `t` odd, `t·n/4 ≡ n/4 mod n/2` ⟹ `ζ^{t·n/4} = ±i`
  ⟹ `|σ_t(α)| = |1 ∓ i| = √2` at **every** embedding ⟹ `house = √2` exactly. This LIFTS the general
  exp-small Habegger/Myerson worst-case floor to a **clean constant** for the dyadic case — a genuine
  (small) structural win of `n=2^μ`. (Unbalanced sums reach house `1`: single root `ζ^j`, a unit; but
  a unit has `|N|=1` and can NEVER be a defect for `p>1`, so the balanced floor `√2` is the relevant
  one. Confirmed: `0` units among balanced α, `min|N| = 2` over balanced α.)

**WHY IT DOES NOT CLOSE / partially-bound the residual — obstruction LOCATED precisely.** A house
**lower** bound is the **wrong direction** to bound the p-defect COUNT:
- A p-defect needs `p | N(α)` ⟹ `|N(α)| ≥ p`; the box gives `|N(α)| ≤ house^φ ≤ (2r)^{n/2}`. So a
  defect EXISTS iff `p ≤ (2r)^{n/2}` (the norm regime, = `n ≤ 64` at prize scale, already known).
  A house **lower** bound only yields `|N| ≥ 2` (rules out `|N|<p`, automatic) — it does **nothing**
  in the prize regime `n=2^40` where `(2r)^{n/2} ≫ p`.
- To FORBID a defect one needs an **upper** bound `house < p^{1/φ}`, but the box already allows
  `house` up to `2r` and that is achieved. The √2 floor constrains only the **shortest** vectors
  (`λ_1`), which — decisively — are exactly the **power-of-2-norm** vectors (`|N| ∈ {2,4,8,…}`,
  `probe_cheng_house_minnorm_407.py`): these can never carry the large odd prize prime and are
  **never defects**. The genuine defect-carriers are the box-**interior, large-odd-norm** members
  (e.g. at `n=8,r=2`: `α` with `|N|=34=2·17` ⟹ a `17`-defect, house `≤ 2r`, sitting INSIDE the box,
  NOT pushed out). The house floor is blind to these.

**Verdict.** The Cheng-house route **reduces to the same box point-count** (framing #10, the
Minkowski/ideal-lattice wall): house bounds `λ_1` (short power-of-2 vectors, non-threatening); the
count is governed by the box-interior large-odd-norm vectors — a **different lattice invariant** the
house cannot see. This re-confirms the wall from the house side and converges with
`probe_407_minkowski_wall_pin.py` (Minkowski FORCES short defect vectors at depth `r ≈ β/2 ≪ ln p`
because `dim 𝔮 = φ(n) = n/2` is huge while `covol = p` is only polynomial). The √2 floor is a
genuine new dyadic fact; it is not enough, and the reason is structural (wrong-direction invariant),
not a missing estimate. Honesty contract: no closure claimed; the partial result is exactly the √2
floor + the located obstruction. Probes: `probe_cheng_house_dyadic_407.py`,
`probe_cheng_house_balanced_407.py`, `probe_cheng_house_minnorm_407.py`,
`probe_cheng_house_floor_proof_407.py`.

---

## 2026-06-13 — REFUTED route: LARGE SIEVE / AVERAGE-OVER-q (explicit-code prize)

**Idea (the explicit-code lever).** The prize lets us CHOOSE the prime q (the code). So instead of
bounding the Gauss-period house for a worst-case q, prove the floor (`κ_r ≤ 1`, equivalently
defect-freeness at depth r) holds for ALMOST ALL primes `q ≡ 1 mod n` in a window `[Q,2Q]`, `Q≈2^168`.
If the bad q are rare, an explicit good q exists. Attack via first/second moment over q.

**The exact object (correctly localized).** A prime q (embedding `ζ_n↦g`, deg-1 prime `𝔮=(q,g−ζ_n)`)
is BAD at depth r iff `𝔮 | α` for some nonzero sparse `α = Σ_{i≤r}ζ^{x_i} − Σ_{j≤r}ζ^{y_j}` (a sum of
`≤2r` roots of unity, `|σ(α)|≤2r` at every place). `𝔮|α ⟹ q | N(α)`. So **the bad primes are EXACTLY
the (correctly-split) odd prime factors of the norm set `{N(α)}`**, capped at `(2r)^{φ(n)}`.

**First moment (union bound), computed.** `#bad q in [Q,2Q] ≤ Σ_{α≠0} ω_{≥Q}(N(α)) ≤ A_r·φ(n)·log(2r)/log Q`,
with `A_r = #distinct nonzero α ≤ n^{2r}`; `#available primes ~ Q/(φ log Q)`. A good q EXISTS when
`Q > n^{2r}·φ(n)²·log(2r)` (the **covering criterion (*)**). Verified numerically
(`probe_largesieve_avgq_407/_deep/_cover`): for FIXED (n,r), once `Q > (2r)^{φ/2}` the defect-free
fraction → 1 (norm regime); below it, a good q (D≤baseline) still exists in 78–90% of the window.

**Why it FAILS at prize scale (three nails, machine-checked):**
1. **(*) reproduces the norm-regime wall, no gain.** At prize params (n=2^32, q~2^256, log_n q=8):
   `log₂Q=256 > 2r·a + 2(a−1)` holds only for `r ≲ 3`. The averaging certifies a good q ONLY up to
   `r ≈ ½log_n Q − 1`, *four times SMALLER* than the per-q norm reach `r_max ≈ 2 log_n q` already
   proven in-tree. The prize needs `r ≈ ln q ≈ 177`. Averaging is strictly WORSE than fixing one q.
   (`probe_largesieve_prizescale_407.py`, `probe_largesieve_finalcount_407.py`.)
2. **The favorable large-norm thinning is a finite-φ artifact.** At small φ (n=8,16) only 0.1–0.5% of
   α have `|N|≈max`, so few α can hit a window-prime — looked promising. But a prime in `[Q,2Q]` is hit
   only by α with `|N(α)|≥Q`, requiring geometric-mean of conjugates `≥ Q^{1/φ}`. At prize `φ=2^31`,
   `Q^{1/φ}=2^{256/2^31}≈1.0000001` — so ALMOST EVERY nonzero α qualifies; `A_r^{≥Q} ~ n^{2r}`. The
   thinning VANISHES, (*) collapses to the raw bound.
3. **Second moment / clustering gives no rescue at scale.** `M2/M1²` (overdispersion) does grow large
   (up to ~188) and `α/badprime` is large (30–290) — real overlap, the union ≪ the first-moment sum —
   BUT only at SMALL primes below the cap (small-norm resonance). Near the window edge Q the defects
   SPREAD (each large-odd-norm α hits its own ~distinct prime), so union ~ sum there: no clustering
   gain at scale Q. (`probe_largesieve_dual_407.py`, `probe_largesieve_secondmoment_407.py`.)

**Convergence with the Cheng-house entry (same wall, new face).** Independently rediscovered the
power-of-2-norm fact: the `min|N(α)|=2` vectors are pure-2-power-norm, NEVER bad for odd q; the genuine
defect-carriers are box-interior large-odd-norm α. The large sieve cannot separate them because at
prize φ essentially all α reach norm ≥ Q.

**Verdict.** The average-over-q / large-sieve route is REFUTED as a path to the floor depth: its
provable reach (`r ≈ ½ log_n Q`) is below the already-proven per-q norm reach (`r ≈ 2 log_n q`), and
both are `~20×` below the prize depth `r ≈ ln q`. The heavy tail (the small-norm α) is a genuine
first-moment obstruction; the second moment quantifies the overlap but the overlap is concentrated at
small primes, not at scale Q. No closure; honesty contract holds. Probes:
