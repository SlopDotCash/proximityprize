...
Assembling the HBK/Stepanov chain. The Mathlib moment identity (`Finset.addEnergy_eq_sum_sq`, via
the
...
This reduces the open sum-product estimate `E(G) ≪ |G|^{5/2}` (⟺ `N ≪ |G|^{3/2}`, dossier §27–28)
to a
**resultant count**: bounding `Σ_c (deg gcd_c)²`. Since `gcd(Xⁿ−1, (C c−X)ⁿ−1)` is nontrivial only
when
`c ∈ G+G` (a common root `ω + ω'` exists), and its degree measures the multiplicity of additive
coincidences, the remaining task is the Stepanov/resultant bound on how the gcd degrees distribute
over
