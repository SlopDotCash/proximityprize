...
section CurveDiffPoly

variable {ι : Type} {F : Type} [Field F]

/-- The coordinatewise difference of the two polynomial-curve evaluations generated by
families `u` and `v`, packaged as a univariate polynomial in the curve parameter:
`diffPoly u v x = ∑ i, C (u i x - v i x) * X ^ i`. Its value at `z` is
`polynomialCurveEval u z x - polynomialCurveEval v z x` (see `diffPoly_eval`). -/
noncomputable def diffPoly {l : ℕ} (u v : Fin (l + 2) → ι → F) (x : ι) : Polynomial F :=
  ∑ i : Fin (l + 2), Polynomial.C (u i x - v i x) * Polynomial.X ^ (i : ℕ)

/-- `diffPoly u v x` evaluated at `z` is the difference of the two curve evaluations. -/
lemma diffPoly_eval {l : ℕ} (u v : Fin (l + 2) → ι → F) (x : ι) (z : F) :
    (diffPoly u v x).eval z
      = Curve.polynomialCurveEval (A := F) u z x
      - Curve.polynomialCurveEval (A := F) v z x := by
  simp only [diffPoly, Curve.polynomialCurveEval, Polynomial.eval_finsetSum,
    Polynomial.eval_mul, Polynomial.eval_C, Polynomial.eval_pow, Polynomial.eval_X,
    Finset.sum_apply, Pi.smul_apply, smul_eq_mul]
  rw [← Finset.sum_sub_distrib]
  exact Finset.sum_congr rfl fun i _ => by ring

/-- The difference polynomial has degree at most `l + 1`. -/
lemma diffPoly_natDegree_le {l : ℕ} (u v : Fin (l + 2) → ι → F) (x : ι) :
    (diffPoly u v x).natDegree ≤ l + 1 := by
  apply Polynomial.natDegree_sum_le_of_forall_le
  intro i _
  exact le_trans (Polynomial.natDegree_C_mul_X_pow_le _ _) (by have := i.isLt; omega)

/-- The `(j : ℕ)`-th coefficient of `diffPoly u v x` is `u j x - v j x`. -/
lemma diffPoly_coeff {l : ℕ} (u v : Fin (l + 2) → ι → F) (x : ι) (j : Fin (l + 2)) :
    (diffPoly u v x).coeff (j : ℕ) = u j x - v j x := by
  rw [diffPoly, Polynomial.finsetSum_coeff]
  have h0 : ∀ b ∈ (Finset.univ : Finset (Fin (l + 2))), b ≠ j →
      (Polynomial.C (u b x - v b x) * Polynomial.X ^ (b : ℕ)).coeff (j : ℕ) = 0 := by
    intro b _ hbj
    have hjb : ¬ ((j : ℕ) = (b : ℕ)) := fun h => hbj (Fin.val_injective h).symm
    rw [Polynomial.coeff_C_mul_X_pow, if_neg hjb]
  rw [Finset.sum_eq_single_of_mem j (Finset.mem_univ j) h0,
    Polynomial.coeff_C_mul_X_pow, if_pos rfl]

/-- If `u · x` and `v · x` differ in some coordinate, the difference polynomial is nonzero. -/
lemma diffPoly_ne_zero {l : ℕ} (u v : Fin (l + 2) → ι → F) (x : ι)
    (hx : ¬ ∀ i, u i x = v i x) : diffPoly u v x ≠ 0 := by
  intro h0
  apply hx
  intro i
  have hc := diffPoly_coeff u v x i
