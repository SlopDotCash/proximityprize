...
axiom weighted_correlated_agreement_for_parameterized_curves [DecidableEq ι]
...
axiom weighted_correlated_agreement_for_parameterized_curves'
    {k l : ℕ} {u : Fin (l + 2) → ι → F}
    {deg : ℕ} {domain : ι ↪ F} {δ : ℝ≥0}
    {μ : ι → Set.Icc (0 : ℚ) 1}
    {M m : ℕ}
...
Then, the words `u` have weighted correlated agreement. -/
axiom weighted_correlated_agreement_over_affine_spaces
    {k l : ℕ} {u : Fin (l + 2) → ι → F}
    {deg : ℕ} {domain : ι ↪ F}
    {μ : ι → Set.Icc (0 : ℚ) 1}
    {M : ℕ}
