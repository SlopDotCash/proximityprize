...
theorem not_DivWeightLe_zero_of_not_dvd_X (x₀ : F) (R : F[X][X][Y])
  ...
  have hW_mk : W𝒪 H = Ideal.Quotient.mk (Ideal.span {H_tilde' H}) (Polynomial.C H.leadingCoeff) :=
    rfl
  have hmk_eq : Ideal.Quotient.mk (Ideal.span {H_tilde' H}) (Polynomial.X : F[X][Y]) =
    Ideal.Quotient.mk (Ideal.span {H_tilde' H}) (A * Polynomial.C H.leadingCoeff) := by
    rw [hEq2, ← hA_mk, hW_mk, ← map_mul]
  have hdif : Ideal.Quotient.mk (Ideal.span {H_tilde' H}) ((Polynomial.X : F[X][Y]) - A *
    Polynomial.C H.leadingCoeff) = 0 := by
    rw [map_sub, hmk_eq, sub_self]
  have hdegX : (Polynomial.X : F[X][Y]).degree < (H_tilde' H).degree := by
    rw [Polynomial.degree_X, Polynomial.degree_eq_natDegree (H_tilde'_monic H hH).ne_zero,
  ...
  have hdegAC : (A * Polynomial.C H.leadingCoeff).degree < (H_tilde' H).degree := by
    ...
    have h1 : A.degree + (Polynomial.C H.leadingCoeff).degree ≤ A.degree + 0 := add_le_add_right
      hdegC _
    have h2 : A.degree + 0 = A.degree := add_zero _
    rw [h2] at h1
    exact lt_of_le_of_lt h1 hdegA
  have hdeg_sub : ((Polynomial.X : F[X][Y]) - A * Polynomial.C H.leadingCoeff).degree < (H_tilde'
    H).degree := by
  ...
  have h_eq_zero : (Polynomial.X : F[X][Y]) - A * Polynomial.C H.leadingCoeff = 0 := by
    ...
    exact h_can.symm
  have h_X_eq : (Polynomial.X : F[X][Y]) = A * Polynomial.C H.leadingCoeff := sub_eq_zero.mp
    h_eq_zero
  -- (Polynomial.X : F[X][Y]) is actually the variable Y in F[X][Y]
  -- We extract the Y-coefficient of both sides.
  have h_coeff : (Polynomial.X : F[X][Y]).coeff 1 = (A * Polynomial.C H.leadingCoeff).coeff 1 := by
    rw [h_X_eq]
...
While `DivWeightLe_zero` is mathematically false because `W𝒪` does not unconditionally divide
  `βHensel 0`,
