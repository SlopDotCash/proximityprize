...
`DBlockIndependence` input.  R25 now proves that input for `d = 8`.
...
`DBlockIndependence` component is supplied by R25. -/
...
theorem superellipticModelHypotheses_eight
  ...
  exact ⟨hmonic, hsf, hdeg,
    dBlockIndependence_eight (gOf u v w c) hsf hdeg hq_odd h8 (hD u v w c hc)⟩
...
theorem dStepanovOutput_of_octic_superelliptic_power_model
  ...
  dStepanovOutput_adapter χ T gOf ζOf hJ hmodel
    (superellipticModelHypotheses_eight T gOf hq_odd h8 hD hpoly)
    he hmq hcount hDtot
