# Proximity-Gap Grand Prize (#407): 100 Ranked δ* Connections

This is the master synthesis of **insightful cross-form connections** for the ArkLib Proximity-Gap Grand Prize (GitHub issue #407 — explicit-RS list-decoding beyond Johnson on a dyadic FFT subgroup μ_n, n=2^μ ⊊ F_q*, q=n^β with β≈4–5, conjectured pin δ*=1−ρ−H(ρ)/(β·log₂ n)). It pools **157 candidate connections mined by 24 lens agents**, deduplicated/merged to corroborated items, re-graded, and ranked by total score (insight+research+relevance) — the **top 100** are kept here. Each connection is a non-obvious structural link bridging ≥2 of the forms F1–F20, linking two walls, wiring in-tree code to an open form, or exposing a hidden symmetry. **Honesty contract:** these are CANDIDATE connections + attack plans, NOT proofs. Nothing here claims closure; every entry labels its open residual. Cited Lean files were Glob-verified to exist on this branch (`claude/113-append-completeness-keystone`); two prose-only paths (`SubgroupGaussSumRawMoment.lean`, `RootSumNormBound.lean`) were dropped as unverifiable.

## Legend

**Forms (irreducible δ*-quantities, all the same wall seen many ways):**
...
| rank | id | total | I/R/Rel | multiform | forms | title |
|---:|---|---:|---|:---:|---|---|
| 1 | C001 | 27 | 9/9/9 | yes | F17 F4 F15 | n-core nonemptiness = NVM minor vanishing = list-decoding-beyond-Johnson shape: F17 / F4 / F15 collapse onto one arithmetic condition n∤a |
| 2 | C002 | 27 | 9/9/9 | yes | F1 F3 F4 | The all-witness ownership floor IS a direct combinatorial upper bound on F3 incidence I(δ) — the δ* object of F1, with NO character-sum input |
| 3 | C003 | 27 | 9/9/9 | yes | F3 F10 F15 | The lacunary variety is a coset-UNION, so I(δ) is an EXACT binomial C(n/2^L, (k+t)/2^L) — not the exponential tower budget |
| 4 | C004 | 26 | 9/8/9 | yes | F1 F3 F9 F10 F15 | δ* count = n·K: the budget q·ε*≈n is exactly ONE μ_n-orbit; eigenstack/monomial-spectrum/lacunary quantization collapse F1=F3=F10=F9 into one counting identity |
| 5 | C005 | 26 | 9/8/9 | yes | F17 F2 F7 F16 | The 1×1 NVM minors ARE the Gauss-period house F2; index-m NVM vanishing = the exact char-sum sup-norm wall |
| 6 | C006 | 26 | 9/8/9 | yes | F2 F7 F5 | The Johnson-vs-capacity gap is an EXTREME-VALUE gap: in-tree code supplies BOTH endpoints [√n proven, √(n log m) conditional on one MGF] |
| 7 | C007 | 26 | 9/9/8 | yes | F1 F3 F4 | The prize wall is EXACTLY where the binomial budget-vs-supply inequality C(2N,r)<r·2^r·C(N,r) flips — a self-contained crossover at r*≈0.3n, no open analysis in its statement |
| 8 | C008 | 25 | 8/8/9 | yes | F12 F3 F2 | Threshold-free per-instance e2=0 criterion: bad α at a=k+2 ⟺ p divides one explicit cyclotomic resultant Res(e2Fold,Φ_{2^m}) — no size threshold, decidable per scale, no BGK |
| 9 | C009 | 25 | 8/8/9 | yes | F1 F3 F4 | The Hab25 K4 cell budget (≤ n) is the EXACT governing-law budget q·ε*≈n — capture cone and δ* law meet at the same n |
| 10 | C010 | 25 | 9/8/8 | yes | F5 F12 F13 | GLT (Garcia-Lorenz-Todd) identity makes the anomaly defect LITERALLY the Hasse-Weil Betti error — W-anomaly = W-Betti by an exact equation, proven at r=2 |
| 11 | C011 | 25 | 9/8/8 | yes | F2 F5 F16 | All three deep-moment walls share ONE root crossover depth r*≈β=log_n q; the deficit is exactly the half-tower height a/2 |
| 12 | C012 | 25 | 9/8/8 | yes | F6 F19 F16 F2 | Hasse-Davenport collapses T_h to the autocorrelation of ONE shifted Gauss-sum sequence (GL(1) monodromy), so the m-fold largesieve dimension wall is mis-applied to the tangent face |
| 13 | C013 | 25 | 9/8/8 | yes | F5 F11 F12 F13 F15 | ONE resultant Res(Φ_n,g)=N(α) is simultaneously the char-p defect threshold (F12), the ideal-SVP / cross-parity norm (F11), and the ZERO-detector of an additive-energy collision (F5/F13) — a single-integer Rosetta stone |
| 14 | C014 | 25 | 9/8/8 | yes | F4 F3 F1 | The missing reverse bound is the witness-spread realizer of the super-code list; two in-tree lemmas already isolate its full content |
| 15 | C015 | 25 | 9/8/8 | yes | F4 F3 | degenerate_stack_no_mcaEvent is the exact countermodel to every list-LB→incidence-LB, pinning the reverse bound to OFF-DIAGONAL stacks |
| 16 | C016 | 25 | 9/8/8 | yes | F17 F4 F15 | The exact genericity gap is the degree-collapse map d↦d mod n; LovettUnionDegreesInjective is stated at the WRONG quotient for μ_n |
| 17 | C017 | 25 | 9/8/8 | yes | F2 F3 F4 | Krawtchouk diagonalizes the Shaw operator: the worst far-line incidence is a Krawtchouk-weighted Gauss period (F3 = F2 ⊗ F4-ball) |
| 18 | C018 | 25 | 9/8/8 | yes | F1 F3 F10 F13 | I_∞(δ) is the q-independent stratified census ∑_j 2^{r-2j}C(2^{μ-1},r-2j); δ*=I_∞^{-1}(n) is a single cyclotomic function — but ONLY above the resultant threshold (prize is below it) |
| 19 | C019 | 25 | 9/8/8 | yes | F2 F5 F13 F16 | The Bessel coefficient bound IS the Salem-Zygmund sub-Gaussian MGF — char-0 closes the chaining input, leaving only the char-p defect |
| 20 | C020 | 25 | 9/8/8 | yes | F2 F3 F16 F18 | CS25 Fourier pair-identity = the LineIncidence spectral identity = the Gauss-period sup-norm M(n), via the SAME dual-code Parseval — cross-cone identification of F2/F3/F16/F18 |
| 21 | C021 | 25 | 9/8/8 | yes | F2 F18 F5 F16 | Autocorrelation r(h)=\|μ_n∩(μ_n+h)\| IS the Fourier square of η_b: F18 flatness ⟺ F2 sup-norm by Parseval, with the diagonal cap already proven |
| 22 | C022 | 25 | 9/8/8 | yes | F14 F3 F10 F15 | The dyadic prize domain is single-prime-power, so the BCH-window/dual-RS code is the COMPLETE & SHARP char-0 case; the prize wall is the char-0→char-p transfer of ONE proven theorem |
| 23 | C023 | 25 | 9/8/8 | yes | F1 F2 F3 F10 | Two distinct group-actions quantize the SAME law: dilation ⟨g^t⟩ on bad-scalars (F3) vs coset μ_n on frequencies (F2); gcd(t,n) via the t-power map is the missing dictionary entry |
| 24 | C024 | 25 | 9/8/8 | yes | F2 F5 F13 | Negation symmetry −1=ζ^{n/2} is load-bearing in OPPOSITE directions: it pins/eases the count faces (F2 real, F5 odd-moment, F13) but IS the wall on the GM-MDS genericity face |
| 25 | C025 | 25 | 9/8/8 | yes | F12 F3 F15 | The parity obstruction is an enumeration-free q-independent KILL of the char-0 e2=0 layer exactly on prize production dimensions k≡0 (mod 4) |
| 26 | C026 | 25 | 9/8/8 | yes | F12 F5 F13 | The cyclotomic norm-defect threshold is ONE engine for both e2=0 rigidity (F12) and the additive-energy clean range (F5/F13): the FINITE-degree escape from F5 vacuity |
| 27 | C027 | 25 | 9/8/8 | yes | F3 F4 F10 F14 | The covering number from the far-count IS the subgroup distinct r-fold subset-sum \|G^{(+r)}\|, with a PROVEN two-sided base-{2,3} bracket [2^{n/2},3^{n/2}] capped by p |
| 28 | C028 | 25 | 9/8/8 | yes | F10 F4 F15 | The KKH26 resultant threshold p>s^{s/2} is the SINGLE shared injectivity boundary controlling both the covering count (F4) and the Half-Sum lacunary vanishing (F10), read in opposite directions |
| 29 | C029 | 25 | 9/8/8 | yes | F5 F2 F13 F12 | The char-p energy SURPLUS E_p−E_∞ is the only open quantity, and it is a Stepanov point-count — not a Weil/moment one (three proven bricks isolate it) |
| 30 | C030 | 24 | 8/8/8 | yes | F1 F3 | The δ* gap H(ρ)/(β log n) IS the entropy-rate of the census numerator: ceiling distance to capacity = (log₂ I_∞)/(β log₂ n) via method-of-types |
| 31 | C031 | 24 | 8/8/8 | yes | F5 F3 F1 | CS25 second moment IS the additive energy E_2; its diagonal/off-diagonal split is the n^{1/2} energy-deficit wall made explicit and machine-checked |
| 32 | C032 | 24 | 8/8/8 | yes | F11 F5 F12 | The proven r=2 Sidon anchor (smallSubgroup_sidonModNeg) IS fully-split ideal-SVP NON-EXISTENCE at the shortest length — extending past r=2 = proving short ideal-lattice vectors don't exist mod 𝔮 |
| 33 | C033 | 24 | 8/8/8 | yes | F5 F10 F12 | The char-p transfer of #lacBad is gated by the SAME ±1-relation height as the BGK energy — one threshold s* governs both faces |
| 34 | C034 | 24 | 8/8/8 | yes | F3 F10 F15 | L must straddle k: the lacunary floor is automatically TRUE when 2^L∤(k+t) (variety EMPTY), FALSE-risk only at the 2-adic carry resonance 2^L\|(k+t) |
| 35 | C035 | 24 | 8/8/8 | yes | F2 F5 F7 | Two-sided reverse-Markov moment-ratio bracket pins F2 to F5's E_r/E_{r−1} growth: the prize is the single law E_r/E_{r−1}≤C·n·log-correction at r≈log m |
| 36 | C036 | 24 | 8/8/8 | yes | F3 F5 F10 F13 F15 | Vieta-pin makes the COUNT face a power-sum-Newton image: I(δ)=#{e_t on {p_1=...=p_{t-1}=0}}, literally the deep additive-energy variety F5/F13 (a char-sum-free F3↔F5 bridge) |
| 37 | C037 | 24 | 8/8/8 | yes | F1 F3 F4 | The granularity ladder (δ*=e/n, Λ linear) and the KKH26 march (δ*=1−r/2^μ, Λ super-poly) are the SAME I(δ) curve below vs above its super-polynomial knee; the prize is the unmapped knee |
| 38 | C038 | 24 | 9/8/7 | yes | F4 F5 F8 | Folding buys an additive-energy reduction: the (d+1)-orbit fold is the multiplicative analogue of the 2-adic energy descent (W-subspace and W-BGK may share one deflation mechanism) |
| 39 | C039 | 23 | 8/7/8 | yes | F2 F20 F19 F7 | The constant-index torsion bound IS the F20 lane with the index t=(q−1)/d as the exact divisor that kills the √-saving |
| 40 | C040 | 23 | 8/7/8 | yes | F5 F2 F16 | The cumulant IS the char-p energy DEFECT and the defect is NEGATIVE: the open core is an UPPER bound free in char 0 that must only not OVER-correct in char p |
| 41 | C041 | 23 | 8/7/8 | yes | F3 F9 | The Möbius/inversion + rotation + monomial-twist generate the full PGL₂-normalizer; the worst-case δ* stack is a SINGLE orbit (reduces the sup to ONE representative) |
| 42 | C042 | 23 | 8/7/8 | yes | F11 F3 F1 | The cross-parity defect A≡−g·B and Conj 1.12's anti-spreading complement are DUAL halves of one fully-split subset-sum image S₀, linking W-Mersenne to W-idealSVP through one set |
| 43 | C043 | 23 | 8/7/8 | yes | F5 F6 F19 | The KU-barrier exponent 1/(d-1) and the moment crossover r*≈β+1 are the SAME conductor-in-the-denominator obstruction on dual faces (L^∞ vs L^{2r}) |
| 44 | C044 | 23 | 8/7/8 | yes | F3 F10 F12 | e_t=(−1)^t·(product of coset values): on coset-unions the bad scalar γ is a PRODUCT in μ_{n/gcd}, collapsing #lacBad ≤ n by a constructive char-p-stable certificate |
| 45 | C045 | 23 | 8/7/8 | yes | F2 F7 F5 | Var=n is literally the Johnson ceiling: the SAME flat-profile equality saturates both Cauchy-Schwarz steps (F7 period variance = W-Johnson tightness) |
| 46 | C046 | 23 | 8/7/8 | yes | F5 F4 F7 | Fourth-moment-can't-beat-Johnson and the period 4th moment are the SAME no-go: any finite L^p moment chain returns the variance, so the escape must be L^∞ (max), the MGF/Chernoff kernel |
| 47 | C047 | 23 | 8/7/8 | yes | F3 F1 F4 | Why NO union/Chernoff bridges average→worst on the line family: MDS first moment ≪n, so the floor is a measure-zero outlier (witness spread exponentially rare) |
| 48 | C048 | 23 | 8/7/8 | yes | F4 F3 | The super-code C⁺=C⊕⟨u₁⟩ collapses worst-case I(δ) into a single rank-(k+1) list-decoding radius, and the cyclic lever forces C⁺=RS[k+1] up to dilation |
| 49 | C049 | 23 | 8/7/8 | yes | F4 F3 F1 | The radius-doubling 2δ in the interleaved collapse is the quantitative form of the 'needs Λ(C)≤O(1)' circularity, and exactly why the MDS-incidence bound is window-vacuous |
| 50 | C050 | 23 | 8/7/8 | yes | F15 F3 F12 | Schur-ladder identity makes e_t the EXACT transducer from the algebraic bad set to a subset-sum set: badSet=−{(k+1)-subset sums}, descending one dyadic level via the squaring fold |
| 51 | C051 | 23 | 8/7/8 | yes | F4 F3 F2 | Curve-decodability ⟹ MCA is char-sum-FREE root counting, so the entire open content is displaced into the close-set cardinality a = the incidence I(δ) (the wall is conserved, not escaped) |
| 52 | C052 | 23 | 8/7/8 | yes | F3 F8 F10 F9 | Imprimitive heavy lines are a self-similar orbit shrink: gcd(d,n)>1 ⟹ eigenratio order n/gcd ⟹ residence in a μ_{n/gcd} sub-tower (F3↔F8 descent, in-code) |
| 53 | C053 | 23 | 8/7/8 | yes | F2 F6 F7 F16 F19 | The Gauss-sum DFT identity η_b=(√q/m)Σ_χ χ̄(b)u_χ makes M(n) an extreme value of an exponential sum with PERFECTLY FLAT amplitudes — N₀ is a pure moment of UNIT phases u_χ, isolating the open core to phase-only |
| 54 | C054 | 23 | 8/7/8 | yes | F3 F2 F5 | The √\|V\| second-moment gap is the q^{n/2} 'union tax' that forces an L^∞ argument (W-Johnson relocated to base-point space via Plancherel duality) |
| 55 | C055 | 23 | 8/7/8 | no | F4 | The proven τ-lower bound IS the s=1 vacuity wall: τ(r)≥ρ−1/n meets the off-range τ=1 collapse — BCDZ25 vacuity is two proven theorems whose ranges fail to overlap |
| 56 | C056 | 23 | 9/7/7 | yes | F15 F9 F3 | Schur vanishing h_{b−k}(x_S)=0 ⟺ esymm-fiber membership, with the dilation acting as h_j(g·x_S)=g^j h_j: the orbit-law multiplier c=g^{b−a} is the homogeneity degree of the controlling complete-homogeneous polynomial |
| 57 | C057 | 23 | 8/8/7 | yes | F2 F5 F10 F18 | Power-sum window = all-ones-error syndrome: e_1..e_{t-1}=0 IS the F2 char-sum incomplete identity at frequency=indicator (Newton bridge F10→F2/F5) |
| 58 | C058 | 23 | 8/8/7 | yes | F3 F9 F10 F15 | Action-orbit (F9) and lacunary-coset (F10) quantization are the SAME ⟨g^{b-a}⟩-coset action — Chai-Fan's K literally equals #lacBad/(coset size), routing the orbit lane into the Schur/esymm-vanishing framework |
| 59 | C059 | 23 | 8/8/7 | yes | F5 F12 F16 F18 | Autocorrelation-at-origin (proven L^∞ brick) × resultant-threshold point-count bounds the char-p energy ANOMALY — fusing the proven caps to localize the open Bourgain-Shkredov factor |
| 60 | C060 | 23 | 8/8/7 | yes | F2 F7 F16 | The Paley-Zygmund lower bound forbids spectral concentration: F2 can hold only by sign-cancellation, never magnitude redistribution — the open core is phase-only |
| 61 | C061 | 23 | 8/8/7 | yes | F3 F4 | List-dimension confinement (dim≤r−1) is the missing reverse bound for the n²-monomial-line concentration — design supplies the rank cap the Poisson floor lacks |
| 62 | C062 | 23 | 8/8/7 | yes | F5 F13 | Stepanov gives a FIXED-r theorem the Weil/AG route cannot (Betti wall): the order-2 auxiliary already beats r(c)≤2 universally over any field — push the vanishing order, not the moment depth |
| 63 | C063 | 22 | 7/7/8 | yes | F5 F2 F7 | Sidon-at-r=2 gives sub-√q but tops out at q^{1/4}√n: the q^{1/4} surplus over √(n log m) quantifies exactly the marginal-vs-extreme-value (max-of-m) deficit |
| 64 | C064 | 22 | 7/7/8 | yes | F2 F3 F7 F18 | The line-incidence spectral collapse + coset-period count give an EXACT identity: per-line incidence error is an n-weighted average of the m periods, so the worst far line equals the worst PERIOD not the worst frequency |
| 65 | C065 | 22 | 8/7/7 | yes | F5 F12 F16 F6 | The cyclotomic norm-defect threshold (2r)^{φ(n)}=p is the EXACT crossover where N₀, E_r, AND the tangent sum T_h all leave their char-0/Jacobi value |
| 66 | C066 | 22 | 8/7/7 | yes | F6 F7 F2 F19 | Kowalski-Untrau vacuity localizes the missing tool to an EFFECTIVE Jacobi-average MGF, not a Wasserstein bound — re-pointing F6/F7 onto the monodromy surface (F19) |
| 67 | C067 | 22 | 8/7/7 | yes | F11 F5 F12 F13 | The cyclotomic norm c^{φ(n)} is a HARD FLOOR on defect onset: it forces the box-vs-sublattice crossover to r*=½p^{1/φ(n)}, identical to CleanRange and the forced-anomaly crossover r*≈β+1 |
| 68 | C068 | 22 | 8/7/7 | yes | F11 F2 F19 | Smoothness↔split-prime is a provable RING-THEORETIC dichotomy (q≡1 mod 2^μ ⟺ μ_{2^μ} exists ⟺ 𝔮 fully splits) — the structural ROOT of why every face hits W-idealSVP |
| 69 | C069 | 22 | 8/7/7 | yes | F5 F8 F12 F13 | The TWO cocycles coincide: the additive tower factor a^{2^k}−b^{2^k}=∏(a^{2^j}+b^{2^j}) IS the Galois norm N(α)=∏_ω g(ω) bounding the char-p energy defect |
| 70 | C070 | 22 | 8/7/7 | yes | F6 F8 F19 | The Azuma √(2 ln m) inflation = the additive-vs-multiplicative gap: replace the additive level-sum cocycle by the multiplicative Jacobi-sum cocycle (Hasse–Davenport tower) with NO bulk-vs-tail loss |
| 71 | C071 | 22 | 8/7/7 | yes | F4 F2 F8 F10 | The full_tower coset-closure budget 2^{n/2^s} and the tower house cocycle are one s-indexed squaring recursion: s=μ (count F4) vs s=1 (house F2), window length t=2^s−1 as the dial |
| 72 | C072 | 22 | 8/7/7 | yes | F8 F18 F10 | Reality from −1=ζ^{n/2} makes the dyadic parallelogram a REAL 2D rotation whose only fixed point is the coherent worst case; ‖A−B‖ (the χ-twisted odd-half period) breaks coherence, tying the gap to Lam–Leung antipodal rigidity |
| 73 | C073 | 22 | 8/7/7 | yes | F9 F15 F3 | The eigenstack orbit law generalizes the dilation: ANY σ-eigenstack reparametrizes badSet by affine T(γ)=a⁻¹b+γ·a⁻¹c — Schur fibers of NON-monomial stacks inherit ord(a⁻¹c)-quantization |
| 74 | C074 | 22 | 8/7/7 | yes | F15 F9 F3 | Additive subset-SUM spectrum (F15, open) vs multiplicative subset-PRODUCT spectrum (proven ≤n): SAME μ_n, opposite collapse — the dilation orbit bridges them |
| 75 | C075 | 22 | 8/7/7 | yes | F5 F13 F7 | The cumulant κ_r is platykurtic (kurtosis→3 from BELOW): a one-sided Markov-Krein with the WRONG-SIGN constraint the SOS no-go ignored |
| 76 | C076 | 22 | 8/7/7 | yes | F13 F5 F16 | Odd-moment NEGATIVITY Σηᵢ^{2k+1}=−n^{2k} is an exact all-r left-skew the even-moment-only Bessel/Markov-Krein machinery throws away |
| 77 | C077 | 22 | 8/7/7 | yes | F5 F7 F13 | E_r/n^{2r}→0 makes the cumulant a TRUE connected correlation: the prize is a free-cumulant (planar) spectral-edge bound, and platykurtosis says the period law is sub-semicircular |
| 78 | C078 | 22 | 8/7/7 | yes | F4 F5 F2 F3 | Cross-face reverse-bound matching: the F4 list-LB (n^{(k+1)/2}) and the F2/F5 4th-moment period-LB (√n) are the SAME tower-word floor — a reverse bound the upper bounds lack |
| 79 | C079 | 22 | 8/7/7 | yes | F2 F3 F5 F16 | The Plancherel collision identity and the Gauss-sum second moment are ONE Parseval at two group levels — making N₀=Σ_b η_b^{2r}/q a literal subset-collision count |
| 80 | C080 | 22 | 8/7/7 | yes | F2 F7 F16 | The proven coset count m=(q−1)/n forces log m not log q — the structural pin making the EVT floor √(n log m) ≠ Ramanujan 2√n |
| 81 | C081 | 22 | 8/7/7 | yes | F12 F13 F10 | The bad locus {e2=0, e1≠0} is provably DISJOINT from all negation/coset structure (e1=0), so the e2 face must use orbit/quadric methods, NOT Lam–Leung antipodal machinery |
| 82 | C082 | 22 | 8/7/7 | yes | F12 F10 F3 | Complement-quadric duality: e2=0 on an a-subset ⟺ its (n−a)-complement satisfies S²=−Q, turning top rows into explicit c-variable quadric point counts where Weil/Deligne is the RIGHT tool |
| 83 | C083 | 22 | 8/7/7 | yes | F12 F5 F9 | Group-action divisibility (S3 ⟹ 6\|M, parity ⟹ even-pair-count, dilation ⟹ ord\|#bad) is a shared 'symmetry kills the count' principle linking e2=0 (F12) to the energy kernel (F5) with NO magnitude bound |
| 84 | C084 | 22 | 8/7/7 | yes | F2 F20 F16 F6 | Two-Gauss-sum hybrid split: the worst-case bound is sharp only because (m−1) of the m completion phases are wasted at √q; the QR closed form is the J=1 effective-phase collapse |
| 85 | C085 | 22 | 8/7/7 | yes | F20 F8 F2 | The index is ODD at the prize, so the constant-index lane can never recurse via a 2-power head: F8 and F20 live on coprime-order sides of F_q*≅μ_n×ℤ/m |
| 86 | C086 | 22 | 8/7/7 | yes | F5 F2 F12 | The proven r=2 Sidon/energy floor E(G)=3n²−3n and its hypothesis are the SAME char-p coincidence wall that gates the all-witness count AND the Gauss-period house |
| 87 | C087 | 22 | 8/7/7 | yes | F4 F5 | explainableCoreSupply_iff_moment makes the named open residual a binomial MOMENT of the agreement spectrum — directly identifying F4 supply with F5 deep moments E_r |
| 88 | C088 | 22 | 8/7/7 | yes | F4 F10 F11 | The covering-transfer hom (surjective + injective-on-G) carries the dyadic obstruction down to every prime quotient AND forward across curve interleaving — one injectivity hypothesis unifies F4, F10, F11 |
| 89 | C089 | 22 | 8/7/7 | yes | F3 F15 F12 | Dual pencil law: collinear (e,m) pair-points are the antipodal/equal-product orbit relations, fusing F3 incidence with the F15 vanishing-Schur face; the open slanted Dickson family is the residual |
| 90 | C090 | 22 | 8/7/7 | yes | F5 F4 F13 | The negative-energy-defect = bounded-support TRUNCATION is the SAME mechanism as the supply LOWER bound B≥Ω(n^{(k+1)/2}): defect sign pins the two-sided bracket via compact support |
| 91 | C091 | 22 | 8/7/7 | yes | F3 F5 F15 | Collision Plancherel = Shaw second-moment: the moment-collision scalar (F5/F15) and the line-incidence Shaw error (F3) are the SAME spectral energy on a different group |
| 92 | C092 | 22 | 8/7/7 | yes | F2 F5 F18 | The energy chain q·E(H)=Σ‖η_b‖⁴ + dilation reduction E=\|H\|·T(H) wires F5↔F2↔F18 into one normalized curve-incidence T(H)=#{(b,c)∈H²:1+b−c∈H} |
| 93 | C093 | 22 | 8/7/7 | yes | F6 F19 F20 | Effective Jacobi equidistribution at CONSTANT index is the Rojas-León homothety lever read backwards: the prize's large automorphism is the multiplicative h-shift, giving √q per-h but only m statistics |
| 94 | C094 | 22 | 8/7/7 | yes | F6 F2 F16 | The Gauss-quotient identity routes the tangent house through gaussSum recursion to a PROVABLE per-h positivity restatement: unit_h·T_h IS an autocorrelation, so its b-DFT is a power spectrum (no L1 bound is tight) |
| 95 | C095 | 22 | 8/7/7 | yes | F6 F12 F19 | The tangent set {1-w:w∈μ_n} carries a forced sign-involution w↦w⁻¹ putting T_h on a curve of degree n/2 (Betti reduction by factor 2 on the multiplicative tangent face) |
| 96 | C096 | 22 | 8/7/7 | yes | F1 F2 F3 | The twisted-inversion GRS-duality monomial automorphism γ↦γ⁻¹ (x·u(1/x)) is a code automorphism the dilation group misses — folds the worst-line search mod an involution, the F2↔F3 swap |
| 97 | C097 | 22 | 7/8/7 | yes | F2 F18 F19 | Per-character Weil energy ‖Tψ‖²≤B SUFFICES (Plancherel bridge built); the missing input is a single Stepanov point-count on the subgroup-restricted curve, not the full BGK sup-norm |
| 98 | C098 | 21 | 7/7/7 | no | F8 F2 | Lyapunov (geometric-mean) bound on ∏r_j escapes the refuted strict-per-level descent: needs only average log r_j ≤ ½log(2 log)+o, WEAKER than the falsified r_j≤√2 at every j |
| 99 | C099 | 21 | 7/7/7 | yes | F3 F5 F10 | Frobenius n\|p+1 gives r(t)≤2 UNCONDITIONALLY — the count face is CLOSED on the inert-prime family, isolating the wall to the split family n\|p−1 |
| 100 | C100 | 21 | 7/7/7 | yes | F13 F7 F8 | Bessel I₀ tail gives a sub-exponential MGF for the period law — a Bennett/Freedman bound the Azuma route could not reach (it used only L^∞ increments) |

## Connections (rank order)
...
**Why insightful:** Three nominally separate forms collapse to one decidable number-theory condition (n∣a) via two axiom-clean in-tree theorems. The cyclic symmetry x^n=1 that makes the domain 'smooth/FFT-friendly' is identified as the EXACT mechanism annihilating the certificate — the smoothness the prover wants is the smoothness the adversary exploits.

**Attack:** LIVE 'n-core reachability crux'. Lean+probe: enumerate window-radius RS[μ_n,k] list-decoding instances and test whether an adversarial rectangle a^L with n∤a is actually REACHABLE (a genuine codeword configuration), not just a formal partition. Build RectReachableInWindow:Prop and either exhibit a reachable nonempty-n-core rectangle (advances the F4 lower bound) or prove window radius forces n∣a (closes the route via HOMDS). Either outcome moves δ*.
...
**Why insightful:** The issue frames the prize as reducing to the BGK wall, but this #371 brick gives a genuinely non-analytic upper bound on the SAME I(δ) — the combinatorial face of the wall that lane A (count) calls for, already axiom-clean in-tree, not conjectured.

**Attack:** Compute the witness threshold w₀ that the prize window δ∈(1−√ρ,1−ρ−Θ(1/log n)) forces (w₀≈(1−δ)n), plug into C(n,k+1)/C(w₀,k), compare to q·ε*≈n. Lean: prize_window_badScalars_le instantiating allWitness_badScalars_card_mul_le at d=k, w₀=⌈(1−δ)n⌉, then Stirling-evaluate the binomial ratio to quantify how far above n it sits — the residual the analytic wall must close.
...
**Why insightful:** The campaign conflated the exponential tower_count free-set bound with the EXACT polynomial coset-union count. The floor only needs the latter, and the coset-union characterization makes #lacBad an exact closed binomial — the open core becomes a binomial inequality in the window, not an analytic cancellation.

**Attack:** Lean: prove vanishingVariety μ_n (k+t) t.card = (n/2^L).choose((k+t)/2^L), L=⌈log₂ t⌉, via full_tower + the 2^L-power bijection (cosetunion ↔ a (k+t)/2^L-subset of μ_{n/2^L}). Pure-arithmetic probe: tabulate C(n/2^L,(k+t)/2^L) vs C·n at window-interior gaps t≈H(ρ)n/log₂(qε*) for μ=10..32, ρ∈{1/2..1/16}; check O(n) vs super-poly. Fully decidable, off the analytic wall.
...
**Why insightful:** Pins the abstract Chai-Fan orbit count K to a concrete computable object (dilation-orbits of subset sums), and reframes the entire δ* threshold as a single integer K crossing 1 — turning the analytic 'is M(n)≤√(n log)' question into a discrete orbit-counting question off the BGK/sup-norm face. The budget n is literally one orbit.

**Attack:** Lean: I_ladder(δ)=n·K(δ)+[0 bad], K(δ):=#(badScalars∖{0})/n well-defined by n_dvd_card_badScalars_adjacent; then K(δ)≤1 ⟺ I≤n+1 ⟺ δ≤δ*. Compose monomial_badSet_mul_invariant with epsMCA_eq_iSup_mcaBadCount. Probe: tabulate K(δ) vs δ across the window interior for μ_8,μ_16 at β∈{4,5}, locate K:1→2 crossing, compare to 1−ρ−H(ρ)/(β log n).
...
**Why insightful:** Pins WHICH minor level (k=1) reduces to the prize char-sum, and shows the top (k=m) minor cft_top_minor_ne_zero NEVER vanishes (det=(det F)²∏G_i, single Cauchy-Binet term, no cancellation). So the entire NVM obstruction lives at intermediate k≈m/2 — a localization the prose ('index 2,3 done, large open') does not make.

**Attack:** Lean: extend NVMDyadicTowerObstruction to prove cft_one_by_one_minor_eq_house : M_{a,b}=(√q/m)·η_{...} as a real equality (currently example:True). Then prove (∀j,|T_j|≤C√(n log m)) → WorstCaseIncompleteSumBound, wiring k=1 NVM-quantitative into the existing GaussPeriodMomentBound consumer.
...
**Why insightful:** Makes precise that 'Johnson→capacity' is not a deeper moment bound but a max-over-m statement: the lower endpoint is already a theorem, the upper endpoint is a maximal inequality whose only open input is a single exponential moment, not all even moments — strictly less than the BGK all-moments wall.

**Attack:** Lean: combine exists_period_sq_ge (floor √n) with chernoff_max_re_le (ceiling √(n log m)) into worstPeriod_bracket conditional on SubGaussianMGF, making the open core = the σ²=O(n) MGF over the m index-m characters. Probe: measure max/√(n log m) and max/√n across n=8..256, proper subgroups, multiple large primes.
...
**Why insightful:** Relocates the open analytic wall onto a fully explicit finite binomial inequality whose low-degree half is already PROVEN (descFact_bulk). The wall is not 'an open theorem about character sums' here — it is 'a binomial ratio exceeds 2^r·C(N,r) once r>~0.3n', checkable and decidable per (n,r).

**Attack:** Probe: sweep (n=2^μ, r) over window radii r≈ρn, tabulate budget/supply; attempt to PROVE the crossover r*(n) in Lean by extending the falling-factorial induction of descFact_bulk past r=√n. Even a non-tight r*(n)≤c·n shows the bulk pin extends below c·n, shrinking the open window. If r* provably above the window edge for some rate, that rate's δ* is pinned outright.
...
**Why insightful:** The standard story is 'threshold c≈n³ needs sharpening'; this reframes the whole face as 'p ∤ one explicit integer Res' — a divisibility condition with no asymptotic threshold, making the q∤D residual a single computable integer per scale, which is why e2 escapes the 25-year BGK wall.

**Attack:** Lean: instantiate pair_sum_collision_dvd_resultant's engine (prime_dvd_resultant_of_common_root + diff_coprime_cyclotomic_rat) for e2Fold to land e2_collision_dvd_resultant and e2_clean_of_not_dvd. Probe: compute Res(e2Fold,Φ_{2^m}) for n=16,32,64 across production rows, factor it, check every odd prime factor < n^β (comment-32 'D is a pure power of 2'); a single odd factor ≥ n^β is a concrete disproof signal.
...
**Why insightful:** Connects the BCIKS20-style capture machinery's terminal constant (n) to the prize budget constant (q·ε*≈n) — they coincide exactly, so Hab25's cell-counting operates at the precise resolution the prize needs, not a coarser Johnson scale. The residual is sharply 'cell count', not 'per-cell size'.

**Attack:** Lean: deltaStar_via_cell_count — if hab25McaBadScalars decomposes into c cells each affine-pinned, then I(δ)≤c·n, so I(δ)≤q·ε*≈n forces c=O(1). Cross-reference gsFactorIndex (Hab25DegreeBudget) to bound c by deg_Y(R); probe (μ_8/F12289 window) whether the bad set ever needs c>1 affine cells (the Hab25 falsify-probe found maximal affine cell a STRICT subset — c>1 occurs; quantify c at window radius).
...
**Why insightful:** The campaign keeps W-anomaly and W-Betti as separate rows; GLT collapses them to ONE — there is no non-Betti object to find (the cumulant-deep-nonbetti verdict) because the cumulant IS the Betti number. Sharpest statement of why every moment route stalls at the same place.

**Attack:** Lean: formalize the GLT r=2 identity V_4=3p(n−1)−n^3 as gauss_period_fourth_moment_eq_fermat_curve_count wired to HasseWeilBoundInstances' Weil bound, giving machine-checked W-anomaly⟺W-Betti at r=2. Probe: _wf407_glt_v4_check across more primes; compute the EXACT crossover r where m^{2r−1}√p first exceeds (2r−1)‼ n^r m and confirm it equals CharSumMomentDeepWall r_max and the cumulant onset r₀.
...
**Why insightful:** The project treats these as three separate barriers; pinning their crossover depths to ONE quantity (a/2=half the tower height) shows they are a single log-vs-log-base-n depth mismatch, sharply narrowing what a winning proof must beat.

**Attack:** Lean: wall_depth_ratio packaging r_opt/r_max=a/2 (CharSumMomentDeepWall) + the (2r)^{n/2} threshold (ManyTermResultantBound) + the kuThreshold (KowalskiUntrauBarrier) as three instances of one RequiredDepthExceedsReliableDepth predicate. Probe: tabulate the three crossover r* across n=2^μ (μ=8..32), β∈{4,5}, confirm coincidence at r*≈β±1.
...
**Why insightful:** The campaign treats T_h as an average of m 'independent' Jacobi sums needing m-fold equidistribution (W-largesieve: f≤√q fails). Hasse-Davenport collapses this to one shifted Gauss sequence — the dimension obstruction quoted as fatal is for the WRONG family. Reframes whether the largesieve dimension wall even applies to the tangent face.

**Attack:** Lean: prove tangentSum_eq_gaussSum_autocorrelation: T_h·(mp)=conj(τ_h)·Σ_{i:χ^{i+h}≠1}τ(χ^i)·conj(τ(χ^{i+h})) using jacobiSum_mul_nontrivial + gaussSum_mul_gaussSum_eq_card (both Mathlib). Probe: max_h|Σ_i τ_i conj(τ_{i+h})| over the SINGLE sequence behaves like √(m·p·log m) (one-sequence autocorrelation) vs m·√p coherent, at p=n^4, n=8..64. If one-sequence autocorrelation governs, the f-dimensional largesieve wall is mis-applied.
...
**Why insightful:** The issue lists F5/F11/F12/F15 as separate faces; this pins them to ONE proven in-tree integer whose three arithmetic properties (=0, p|·, magnitude) ARE the faces, and shows the cross-parity/ideal-SVP wall is the geometric dual of the energy-anomaly threshold.

**Attack:** Probe _wf_resultant_triple_face.py: for prize (n,p) enumerate small signed g, compute R=Res(Φ_n,g), verify the readings co-occur (R=0↔energy collision; p|R↔mod-p collision; (α,𝔭) ℤ[ζ_n]-vector L1-norm vs Minkowski for SVP). Lean: extend with idealNorm_eq_resultant linking N(α) to the F11 ideal generator and three wrappers (F5/F12/F15 defect tests each reduce to p|Res via dvd_resultant_of_isPrimitiveRoot_isRoot_bound).
...
**Why insightful:** Reframes 'no matching reverse bound' (issue §3 face-1) from a vague gap into a precise, already-half-formalized obstruction: the list-to-incidence map fails not on cardinality but on witness-set distinctness. The two MCAWitnessSpread lemmas were filed as lower-bound necessity but are exactly the reverse-bridge skeleton.

**Attack:** Lean: incidence_ge_list_of_distinct_witnesses — assume each RS[k+1] δ-list codeword has X^k-coeff giving distinct γ and a witness set S_γ failing pairJointAgreesOn; conclude |mcaBadSet|≥|list| by composing epsMCA_ge_card_div_of_mcaEvent_set with the contrapositive of common_witness_badGamma_set_card_le_one. Probe (μ_8/μ_16 over F12289): do the worst-line δ-list codewords realize distinct agreement sets at window radius, or cluster?
...
**Why insightful:** Shows the in-tree reverse list bound is real but mis-targeted, and identifies the single structural fix (non-degenerate stack, same averaging-list count). Sharper than 'no matching reverse bound' — a constructive demand, not an absence.

**Attack:** Lift averaging_list_lower_bound from (g,0) to (u₀,X^k) via badScalars_monomial_eq_degreeLTSucc: demand the C(n,k+t)/q^t agreeing degree-<k+1 polynomials have DISTINCT X^k-coefficients (distinct bad γ) and non-matchable second row. Probe (μ_16/F12289): count how many averaging-class codewords have distinct leading coeff and fail joint-agreement — measure the loss from list-count to bad-scalar-count on a real non-degenerate stack.
...
**Why insightful:** Turns the abstract 'genericity barrier' into a single concrete arithmetic map, and shows the in-tree Lovett residual is at the wrong quotient: closing LovettUnionDegreesInjective certifies generic RS but is provably silent at μ_n. Precise reason 'finishing LovettPrimitiveStep does not win the prize.'

**Attack:** Lean: state SubgroupLovettUnionDegreesInjectiveModN (require |Vᵢ|+e distinct MOD n) and prove ...ModN → pFamUnion_linearIndependent over μ_n via genVandermonde_rootsOfUnity_det_ne_zero_iff instead of natDegree injectivity. First honest μ_n-specific GM-MDS reduction; hypothesis is exactly n-core emptiness.
...
**Why insightful:** Exhibits F3 as the literal convolution of F2 and F4 via the Krawtchouk transform, all three bricks proven axiom-clean. The Krawtchouk weight is the EXACT object §3's face-1↔face-2 reverse bound was missing, and its sign-oscillation could give cancellation the raw |η_b| sup-norm misses.

**Attack:** Land incidence_eq_krawtchouk_gausssum composing incidence_eq_average_add_shaw + a ball-version of shawError_subgroup_eq (replace [ψ∈H^perp] by Σ_{k≤δn}K_k(wt ψ) via shell_fourier). Probe (μ_8/μ_16, p≈n^4): evaluate Σ_{k≤δn}K_k(wt ψ)·η_b vs direct incidence to confirm; measure whether the Krawtchouk weight DECAYS high-|b| periods (new lever for the reverse bound).
...
**Why insightful:** Reframes the 'q-independent single function' claim HONESTLY: the q-independent census is the large-p envelope, and the entire prize difficulty is localized to char-p surplus collisions BELOW the resultant threshold (W-LamLeung). Staircase restatement of the same char-p crux, as a count not a sup-norm.

**Attack:** Probe: for fixed (μ,r) sweep p across [n^4,(2^μ)^{2^{μ-1}}], measure I−I_∞ vs p; surplus governed by #{(S,T):p|N(λ_S−λ_T)}=resultant-divisibility count. Lean: bound the char-p surplus by (collisionPairs μ r).card·(largest resultant)/p feeding KKH26FixedRResultantBound (2r)^{2^{μ-1}}, show it stays ≤n at β≈4 (count-side of the prize, off BGK).
...
**Why insightful:** The frontier files name the MGF as OPEN and the moment bound as a separate route, but bessel_energy_le_gaussian already proves the generating-function form of the MGF in char 0 — nobody wired the Bessel brick into SubGaussianMGF. Collapses the F5/F16 duality to one inequality whose only gap is the char-p transfer.

**Attack:** Lean: prove SalemZygmundChaining.SubGaussianMGF η n for the char-0 diagonal model from bessel_energy_le_gaussian by summing the coefficient bound against λ^{2r}/(2r)! (Carleman/MGF assembly) → besselEnergy_implies_subGaussianMGF; compose with chernoff_max_re_le for B_char0≤√(2n log m), isolating the residual as Σ_r(E_r−E_r^{(0)})λ^{2r}/(2r)! controlled by CyclotomicNormDefectThreshold.
...
**Why insightful:** CS25's identity is universally read as averaging; recognizing it is the W4/Gauss-period Parseval (identical to LineIncidenceSpectral) shows CS25 already contains the prize's spectral object — and explains WHY CS25 only reaches Johnson: it bounds L² mass (free) but the prize needs the single worst ‖Ŝ(ψ)‖, which CS25 discards.

**Attack:** Lean: cs25_fourier_offdiag_eq_lineIncidence_spectralError instantiating fourier_pair_identity with C=ker of line-evaluation and B=syndrome ball, rewrite via lineIncidence_spectral to expose ∑_{ψ⊥s₁}‖Ŝ(ψ)‖². Probe (μ_8/μ_16, p=12289): compute both sides, confirm the worst dual frequency = measured M(n); lets any future M(n)≤C√(n log(p/n)) feed CS25's E[N²] through a proven bridge.
...
**Why insightful:** Identifies F18 as the EXACT Fourier transform of F2 with the spatial-side bound already landed, making the duality a precise Parseval statement; and pinpoints why the proven dead-simple Cauchy-Schwarz lemma does NOT close the prize: the weight's sign.

**Attack:** Lean: autocorr_fourier_eq_eta_sq: r̂(b)=‖η_b‖² (orthogonality), then F18 as max_b r̂(b)≤n·L ⟺ PaleyFloorBound. Probe: compute r(h), verify r̂(b)=|η_b|² to machine precision; state the open flatness as max_h|Σ_j τ_j conj(τ_{j+h})|≤√p·√(m log m), citing autocorr_le_autocorr_zero as the nonneg-weight degenerate case it fails to reach.
...
**Why insightful:** Not stated anywhere that the dyadic prize lands in the simplest fully-solved radix of the in-tree window machinery, and that the only missing ingredient is char-0→char-p transfer of a SINGLE proven theorem. Reframes the wall as a transfer problem with a known char-0 anchor.

**Attack:** Lean: dyadicWindowMinWeight_charP:Prop = (the least-divisor-of-2^μ-exceeding-t min weight survives mod q), prove weighted_windowed_prime_pow_charP → DyadicLacunaryFloor by reusing the char-0 coset-indicator classification as the candidate set. Probe: re-run the prime-power window-law census over F_q with q≡1 mod 2^μ (split, β≈4) for μ=3..6, counting char-p-only window-vanishing vectors that are NOT char-0 coset unions (the defect carriers); measure ≤q·ε*≈n.
...
**Why insightful:** The campaign treats F2 (period count) and F3 (incidence count) as the same '~n quantization', but they are governed by two genuinely different actions whose ratio is gcd(t,n); the t-power map g↦g^t is the unnamed bridge, and it predicts the even/odd-t parity split in incidence directly.

**Attack:** Lean: lacBad_orbit_count: #lacBad/(n/gcd(t,n))=#{distinct ⟨g^t⟩-orbits}; probe (extend probe_lacbad_crossover_407.py) measuring #orbits as a function of gcd(t,n) at the window edge to test whether the ORBIT count is O(1) exactly when gcd(t,n)=1. If so, the floor reduces to coprime-t directions and the t-power collapse handles the rest by self-similar μ_{n/2} reduction.
...
**Why insightful:** The literature treats 'negation helps the count' and 'negation blocks GM-MDS' as unrelated facts; recognizing they are the SAME ζ^{n/2} acting with opposite valence explains why the prize is doubly-barriered (count-tractable yet genericity-blocked) and tells you which face a new symmetry must respect.

**Attack:** Make the dichotomy a theorem: take the even-polynomial certificate from MuTwoPowDerandRefutation and show its agreement set is a union of antipodal pairs (freePart=∅), i.e. the GM-MDS counterexample is in the kernel of the KKH26 sum-invariant. Lean: derand_certificate_is_paired (bad RIM-kernel words negation-closed). Proves the genericity obstruction and the count-collapse have a common cause (paired support); the prize floor must be sought on the freePart locus where BOTH the count is governed AND genericity is not automatically saturated.
...
**Why insightful:** Converts the hardest object (bad-scalar count) into a one-line mod-2 parity check on C(k+2,2), bypassing enumeration and character-sum machinery, landing precisely on the dyadic FFT dimensions k=2^j — a structural reason the depth-1 layer is silent that no analytic route sees.

**Attack:** Lean: wire e2_ne_zero_of_production_dim into the comment-40 e2=0/α=−1/e1 reduction to land depth1_badScalarSet_empty_of_production_dim. Probe: compute the coefficient-sum mod small primes of the folded e_t relation for |A|=k+t over n=16,32, look for an arithmetic-progression of clean (k,t) pairs — if the parity pattern persists for t≥2 it kills whole diagonals of the staircase for free.
...
**Why insightful:** Exposes that the 'no-BGK-wall' property of F12 is exactly the FINITE-DEGREE escape from F5 vacuity: both use the identical Norm≤M^{φ(n)} engine, but e2's relation is r-uniform so its threshold doesn't blow up with depth — the one structured corner where the cyclotomic-norm method is non-vacuous in the prize regime.

**Attack:** Lean: refactor WindowTwoLayer's threshold to be a direct corollary of prime_le_of_cyclotomic_signed_sum (unifying the two resultant engines into one Norm≤M^φ lemma). Research probe: tabulate Norm(e2 relation) vs (|A|²)^{φ(n)} for small n to measure the gap between crude and true threshold (≈n³ measured vs (n²+n)^{n/2} provable); if the true norm spectrum is poly(n), the e2 face is clean unconditionally at prize scale.
...
**Why insightful:** The covering/incidence count is NOT analytic in char 0 — it is doubly-exponential and fully pinned combinatorially; the ENTIRE prize-relevant subtlety is the field cap p truncating it. Re-frames 'is the bad count ≤ budget' as 'does the mod-p reduction collapse the signed cube below n', a decidable arithmetic question, not BGK.

**Attack:** Probe: for μ_n over prize-shaped primes p=Θ(n^β), β∈{4,5}, compute #{distinct ∑_{i∈S}ζ^i:|S|=r} at r=ℓ near the window vs both 2^r·C(n/2,r) (char-0) and the budget n. Lean: RealizedCoveringCount G r:=(sumsetDistinct G r).card, prove epsMCA≥RealizedCoveringCount/q by wiring FarCosetExplosion to the §7 stack.
...
**Why insightful:** Exposes the floor (F10) and the ceiling/counterexample (F4 via KKH26) as the SAME resultant inequality flipped — p>s^{s/2} gives the ceiling, p≤s^{s/2} would give the floor, and the prize p=n^β sits in the crossover where (2r)^{n/2} vs p is delicate. A concrete decidable hinge rather than the diffuse BGK wall.

**Attack:** Quantify: at the prize p=2^{βμ}, threshold s^{s/2}=2^{μ·2^{μ-1}}≫p for n≥16, so KKH26 distinctness FAILS to be guaranteed by the size bound exactly in the prize regime (why kkh26_lemma1_of_not_dvd replaces it via Thorner-Zaman good primes). Lean: LacunaryFloorClean G r:=∀ short signed R, R(g)≠0; prove ⟺ ¬p∣collisionResultant (not_dvd_collisionResultant_* half-exists), unifying F10 and F4. Probe: for n=8,16,32 and p=Θ(n^4), count how many pairwise collision resultants are divisible by p (the TZ24 good-prime density).
...
**Why insightful:** Surgically isolates the open quantity (char-p surplus) from the fully-governed char-0 part using THREE proven bricks, and reclassifies it from an L^∞/BGK sup-norm object (where the campaign is stuck) to a point-count object. The campaign notes 'the char-p anomaly is the only open part' but treats it as a moment that explodes; this frames it as a variety-point-count to Stepanov-bound directly.

**Attack:** Lean: energySurplus G p r:=E_r^{F_p}−E_r^∞ as a Prop, prove E_r^{F_p}=E_r^∞+surplus with surplus=#{(x,y)∈μ_n^{2r}:Σx≡Σy mod p, Σx≠Σy in ℤ[ζ_n]}. Probe: count surplus_r directly at prize instances (n=64, β=4) to confirm surplus_r=O(n^{2r}/p) (Sidon-spread, sub-floor). Target a brick surplus_r≤(2r)^{n/2}-type norm criterion for the LOW-r band, isolating the genuinely-open mid band r∈(β/2, ln p).
...
**Why insightful:** Shows the headline constant H(ρ)/β is not mysterious-analytic but forced by C(n,ρn)≈2^{nH(ρ)} method-of-types on the census — reducing the prize constant to a binomial entropy fact already in-tree, isolating exactly the two binomial-tail inequalities.

**Attack:** Lean: discharge the two entropy-gate walls in the prize regime via the in-tree binomial sandwich (n+1−a)^a≤a!C(n,a)≤n^a — upper wall is the truncation collapse; lower wall needs H(a/n)·n≥(a−k)β log n = two_rpow_entropy_le_succ_mul_choose at a=ρn+gap. Then mcaDeltaStar_le_of_entropy_gate gives the upper-bracket ceiling for the GENERIC ladder code unconditionally — a real advance on the upper side.
...
**Why insightful:** The issue treats 'CS25 second moment' and 'the E_r energy wall' as separate; this shows the CS25 ball-intersection cone literally computes E_2 of the RS code, and its required diagonal-separation is the SAME structural fact _MomentMethodNoGo records.

**Attack:** Lean: prove ballInterCount r 0=V and ∑_{v∈RS∖0}ballInterCount r v=E[N²]/|RS|−V exposing codeAdditiveEnergyOffDiag. Formalize that optimal-θ of (q·(2θ+(q−2)θ²)+(1+(q−1)θ²))^n/q^{n−k} equals the entropy-volume off-diagonal; probe (μ_8/μ_16, p=12289, β≈4.5) off-diag/diag at WINDOW radius is Θ(1), testing whether E[N²] reaches the floor or stalls at Johnson.
...
**Why insightful:** The campaign never frames its OWN proven Sidon/norm bricks as ideal-SVP results; treats ideal-SVP as an external wall. This shows the in-tree axiom-clean lemmas are the r=2 instance of the very Pan-Xu-open quantity, so the prize is a quantitative continuation of code that already exists.

**Attack:** Probe (probe_407_sparse_support_ideal_svp.py exists): extend to track min_house(r), confirm min_house>4 forces r=2 cleanness exactly when 4^{φ(n)}<p. Lean: generalize smallSubgroup_sidonModNeg to smallSubgroup_noSparseDefect_of_lt for 2r terms (= no_spurious_tuple_of_lt_prime specialized to the energy g); state the OPEN residual as SparseIdealSVPNonExistence n p r and show it ⟹ the energy clean range, isolating the Pan-Xu gap as one named Prop above the proven r=2.
...
**Why insightful:** The KB treats the count's char-p transfer and the energy's char-p transfer as separate residuals. This shows they are the identical {-1,0,1}-lattice-relation-mod-q question with one explicit threshold s*. Converts two 'open inputs' into one, gives a concrete decidable predicate.

**Attack:** Probe: for prize primes q≈n·2^128, test whether μ_{2^μ}-th roots admit any nonzero {-1,0,1} relation of length ≤s* mod q (LLL on the cyclotomic lattice of ζ_{2^μ} mod q). Lean: RelationFreeAtLevel q μ s* := (∀{-1,0,1}-vector c of support ≤s*, ∑c_jζ^j≠0 in F_q); prove DyadicLacunaryFloor ∧ GaussianEnergyBound BOTH follow from it.
...
**Why insightful:** Explains WHY the measured incidence is Θ(n) and quantized in units n/gcd: the only live gaps are 2-power-resonant with k+t, and at the prize window 2^L≈t so n/2^L is only ~log n — a SMALL ambient, forcing the binomial polynomial. A new reduction of the floor to a 2-adic carry/divisibility condition on (k+t), invisible to the analytic faces.

**Attack:** Probe: for prize gaps t₀=⌈H(ρ)n/log₂(qε*)⌉, compute L=⌈log₂ t⌉, n/2^L, and C(n/2^L,(k+t)/2^L) only at resonant t (2^L|(k+t)); verify it never exceeds C·n. Lean: lift card_le_gcd_of_esymm_zero to the lacunary case via μ_{2^L}-closure of full_tower (the usable content is the nonemptiness divisibility 2^L|(k+t)).
...
**Why insightful:** The lower companion is in-tree but read only as 'optimality of √n'. Reading it together with the upper feeder at the SAME r exposes that the Johnson→capacity gap is precisely the growth of E_r/E_{r−1} with r — the extreme-value law in disguise — a sharper two-sided target than the one-sided BGK sup-norm. The ratio telescopes away the (2r−1)‼ prefactor, a cleaner object.

**Attack:** Probe E_r/E_{r−1} directly (closed form via Bessel E_r=(2r)![x^r]I₀(2√x)^{n/2}); locate where it departs from ≈n in the prize regime, cross-check against W-anomaly crossover r*≈β+1. Lean: combine the upper WorstPeriodMomentBound and lower exists_period_sq_ge_moment_ratio into worst_period_bracket_of_energy_ratio (hratio:∀r,E_r≤C·n·E_{r−1}) : M(n)≤√(C·n·log...), isolating the energy-ratio as the one named open Prop.
...
**Why insightful:** The issue links F3 (count) and F5 (energy) only through the char-sum moment Ση^{2r}=qE_r; this gives a SECOND char-sum-free bridge via Newton's identities, turning #lacBad directly into a vanishing-power-sum subset count — the object EnergyRelationAntipodal already controls in char 0.

**Attack:** Lean: lacBad_eq_powerSumVanishing rewriting {e_1=...=e_{t-1}=0} as {p_1=...=p_{t-1}=0} via Mathlib NewtonIdentities (Multiset.esymm↔psum), then a probe counting a-subsets of μ_n with p_1..p_{t-1}=0 and checking it equals the brute-forced bad count and is O(n) at prize radius. If the antipodal-balance lemma forces these into ⟨g^t⟩-cosets, that is DyadicLacunaryFloor.
...
**Why insightful:** Unifies the two separate 'pin' families as one I(δ) profile and LOCALIZES the open core to the knee: the prize is not the top (march, proven bad-side) nor the bottom (ladder, proven), but the single transition radius where I(δ) first exceeds n. A concrete geometric reframing of 'sup{δ:I(δ)≤n}' as 'locate the knee'.

**Attack:** Probe: at a good prime (μ=5, β≈4) compute I(δ) at every grid radius, identify δ_knee where I first exceeds n=q·ε*; check whether it matches 1−ρ−H(ρ)/(β log n). Lean: prove I(δ) nondecreasing (epsMCA_mono) PLUS a single intermediate bracket I(1−√ρ)≤n (Johnson side) — combined with the march bad-side, bracket δ* into one rung [1−√ρ,1−r*/2^μ], r* smallest with 2^r·C(2^{μ-1},r)>n.
...
**Why insightful:** The issue treats W-subspace (list) and W-BGK (char-sum) as DOUBLY-independent barriers. This proposes they share one mechanism — the orbit-fold deflation factor — which would mean a single energy-descent inequality governs both walls. Explicitly contrary to the 'two unrelated walls' framing and, if true, high-value (if false, a publishable refutation).

**Attack:** Probe: for μ_n over a prime field compute (i) plainAgree/foldedAgree for x↦x² and (ii) M(n)²/M(n/2)² (char-sum), across n=8..128, test whether both track (d+1)=2 with the same sub-maximal correction. If corrections coincide, formalize foldedAgree_le ↔ charSum_descent reducing both walls to one per-level deflation Prop. If they diverge, the 'two walls' framing is confirmed.
...
**Why insightful:** Reads the in-tree worst-case proof (normally 'B≤√q') as the F20 constant-index bound, exposing the saving as a single +1/t term over the cofactor — making BurgessIndexOvershoot's √(m/log m) factor a direct consequence of one proven inequality, and identifying the lane parameter with the Paley eigenvalue count m.

**Attack:** Lean: restate as eta_le_constIndex: ‖η_b‖≤√q−(√q−1)/t (one-line algebra), companion constIndex_saving_eq:(√q−bound)=(√q−1)/t, corollary prize_saving_negligible instantiating t=(q−1)/n, q=n^β to show saving<1 whenever m>√q (the prize). Converts the integer surrogate of BurgessIndexOvershoot into the exact real saving formula.
...
**Why insightful:** CharSumMomentDeepWall and the cumulant-not-moment KB say opposite-sounding things and both are right (one raw, one connected). The reconciliation — subtract n^{2r} FIRST, then bound — is the correct order of operations and dissolves the W-anomaly 'forced positive' obstruction at the cumulant level.

**Attack:** Lean: CumulantBound G r := (q·rEnergy G r−(G.card)^{2r}:ℝ)≤q·(2r−1)!!·n^r; derive B^{2r}≤cumulant+n^{2r}-cancellation via the existing single_le_sum arrow restricted to b≠0 (one-line edit of term_le_moment over univ.erase 0); show CumulantBound at r≈ln q ⟹ B≤√(2n ln q). Carry as a SIGNED cumulant, matching the negative-defect measurement.
...
**Why insightful:** The literature uses only affine+permutation symmetry; the inversion (Möbius) generator + diagonal twist fuse the extremal set into one orbit — much stronger than 'extremal lines are monomial directions', giving a canonical extremizer to compute against.

**Attack:** Lean: assemble mcaEvent_rs_rotate, inversion, monomial twist into mcaEvent_pgl2_normalizer_invariant, strengthen epsMCA_eq_iSup_subtype_of_reps to a sup over PGL₂-normalizer-orbit reps. Compute the orbit of the canonical (x^k,x^{k-1}) extremizer; show every window-extremal stack is in it (probe μ_8/μ_16 multiple primes). If unique, δ* is its single bad-coset count.
...
**Why insightful:** Makes the prose 'anti-spreading complement of Conj 1.12' a precise set identity: both are statements about |S₀| and |S₀∩(−g)S₀| of the SAME subset-sum image, and the in-tree ⟨−2⟩ covering exhibits the extreme.

**Attack:** Probe: for μ_n⊊F_q* (q=n^β, β∈{4,5}), compute (a) |S₀|/q (Conj-1.12 spreading) and (b) |S₀∩(−g)S₀|/n (cross-parity defect/budget); test anti-correlation across primes, and whether worst (b) occurs when (a)→1. Lean: CrossParityDefectEqDilateIncidence defining #defect=|S₀∩(−g)S₀|, prove the Mersenne-covering case gives full F_q, reusing mem_sumsetDistinct_signedPowersF.
...
**Why insightful:** Shows the equidistribution wall and the deep-moment-anomaly wall are not two barriers but one invariant (subgroup-size-in-exponent vs O(1)-power field) on the L^∞ (KU/Jacobi) and L^{2r} (moment) faces. Unifies W-largesieve, W-anomaly under one inequality n vs β log n.

**Attack:** Lean: size_in_exponent_obstruction parameterizing both — given budget log q=β log n and a tool needing (size)·c≤log q, derive vacuity for size=n, d=2^32. Reuse the KU omega-style proof shape with the moment threshold r_max=2 log_n p. Probe: tabulate KU rate-exponent 1/(d-1) against moment crossover r* across the same (n,q) grid to confirm they cross the budget at the same scale.
...
**Why insightful:** Turns 'image collapse' into a concrete product formula: the bad scalar IS a product of coset-class values in a subgroup of order ≤n. The structural reason #lacBad≤n (the budget) at the smallest live gap, and a constructive char-p-stable certificate (products of roots of unity), not an analytic estimate.

**Attack:** Lean: prove for t=2^L (smallest live gap dividing k+t) that lacBad μ_n (k+t) t ⊆ a single coset-orbit of μ_{n/2^L} via γ=±∏x_i^{2^L}, hence #lacBad≤n/gcd(t,n)≤n. Probe: enumerate γ-values for coset-union S at small μ_n, confirm one ⟨g^t⟩-orbit.
...
**Why insightful:** The issue calls these 'faces of one wall' but never pins the EXACT shared equality. The flatness witness in cauchySchwarz_eq_iff_flat is the same configuration as the period spectrum being flat (Salem-Zygmund extremal), tying F7's Var=n to W-Johnson's tightness lemma via one Finset identity.

**Attack:** Lean: periodVariance_flat_iff_johnsonExtremal applying cauchySchwarz_eq_iff_flat to the ‖η‖-derived profile, showing the CS-equality set = {η : spectrum flat}; isolates the above-Johnson content as strictly the non-flatness (decorrelation). Probe: on μ_8/μ_16 check the per-coordinate matchCount profile for a far line is flat exactly when the period family is L²-extremal.
...
**Why insightful:** Unifies a list-side no-go (JohnsonFourthMomentNoGo) with a character-sum-side lower bound (WorstPeriodLowerBound) under one principle: any FINITE-degree moment chain returns the variance. Sharpens §7.2 to a structural reason — the extreme value is L^∞, invisible to every L^p — and points to the MGF/Chernoff kernel as the unique admissible tool.

**Attack:** Lean: meta-lemma finiteMoment_returns_variance abstracting both squaredJohnson_le_fourthChain and the period 4th-moment chain over an arbitrary nonneg profile: any (S_{2r})^{1/r} bound is ≥ the variance, equality iff flat. Cite chernoff_max_re_le as the only escape. Probe: confirm period kurtosis→3 at proper subgroups, certifying no L⁴ gain.
...
**Why insightful:** Connects F3 (line incidence) to F4 (list size) by exhibiting IN CODE that the maximizer is structurally rare relative to the MDS average, so the 'average→worst is the crux' clause (§7.3) is a genuine extreme-value obstruction, made concrete by the explicit Θ(n²) spread sitting astronomically below the q·ε* budget.

**Attack:** Lean: firstMoment_vs_floor_gap combining line_first_moment_bound (avg≈poly(n)/q) with the explicit spread bounds to state E_line[I]≪I(δ*)≈n, certifying no Chebyshev concentration can certify the floor. Probe: at q=12289, μ_8/μ_16, tabulate avg vs max line incidence over primitive monomial directions, confirm max/avg→∞.
...
**Why insightful:** Explains why the prize is RS-list-decoding specifically and not a generic super-code list problem: the dilation symmetry PINS the worst super-code to RS[k+1]. The favorable direction of the genericity wall (W-genericity usually blocks; here it RESTRICTS the adversary's super-code to a structured one).

**Attack:** Lean: worstSuperCode_eq_RSsucc combining explainableScalars_card_le_superList (max over super-codes) with FarLineIncidenceEquivariance + badScalars_monomial_eq_degreeLTSucc to prove worst-case I(δ) is achieved at u₁=X^k, super-code=RS[k+1]. Probe whether any non-monomial far direction beats RS[k+1]'s list at window radius (μ_16/F12289).
...
**Why insightful:** Ties three in-tree facts (radius-doubling necessity, MDS-incidence vacuity threshold (1−ρ)/2, sandwich collapse) into one mechanism, giving quantitative meaning to the qualitative 'circular' verdict. The (1−ρ)/2 half-window threshold is the same place the unconditional bracket bottoms out.

**Attack:** Lean: interleaved_collapse_window_vacuous — at any δ in (1−√ρ,1−ρ), the floor 2t−n<k, so interleavedList_card_le gives C(n,k)/C(2t−n,k)≥q (exponential), certifying nothing. Contrapositive: a window-radius MCA bound via the collapse REQUIRES a sub-C(n,k)/C(a,k) (higher-order-MDS) list bound at half-radius — quantify the exponent gap as a function of (ρ,β,n).
...
**Why insightful:** Pins the precise mechanism turning algebraic e2/e_t vanishing into a concrete additive subset-sum count, and shows the t=2 case descends to a t=1-shaped count on the squared subgroup — making the dyadic squaring tower the literal recursion under the whole bad-set magnitude question.

**Attack:** Lean: combine residual_ladder_schur + esymm2_inflate to prove the recursion badCount(μ_n,t=2) reduces to squareSubsetSumCount(μ_{n/2},t=1) and iterate ⌈log2(t+1)⌉ times (the DISPROOF_LOG window-halving engine). Probe: measure whether the per-level subset-sum spread stays O(n) down the tower at window-interior gaps t≥t0.
...
**Why insightful:** Pinpoints WHY the curve route is not a free escape from BGK even though it never touches a character sum: the degree-ℓ root bound is exact and lossless, so the wall is conserved and reappears as the close-set count. Contradicts the natural hope that an algebraic MCA proof sidesteps the analytic core.

**Attack:** Lean: curveCloseSet δ u f (GG25CurveDecodability) = badScalars-style object for ℓ=1, show its card equals I(δ) of F1. Then a named Prop CurveCloseBudget:=a≤q·ε*; all_seeds_close_of_curveDecodable already discharges downstream, so the only open input is CurveCloseBudget=F3=the wall. Probe (μ_8/μ_16): tabulate #curveCloseSet at window radius for monomial stacks, confirm it equals the spectral incidence count.
...
**Why insightful:** Reframes the imprimitive-heavy-line phenomenon (currently a numeric observation) as an exact proven orbit-order computation: heaviness = the eigenratio dropping to a proper subgroup, i.e. the F8 2-adic descent realized as a change of which μ_{n/gcd} the orbit lives in — not a separate analytic fact.

**Attack:** Lean: generalize monomial_badSet_mul_invariant to (x^a,x^b) with gap d, proving the eigenratio is g^d of order n/gcd(d,n), hence n/gcd(d,n)∣#bad. Budget lemma: if #bad>0 at the window, #bad≥n/gcd(d,n); heavy imprimitive = gcd(d,n)>1 forcing residence in a μ_{n/gcd} sub-tower bounded by the F8 descent. Probe: confirm n/gcd(d,n)∣#bad for all directions at μ_16 over a large prime, and overflow happens iff multiplicity·(#cosets)>budget.
...
**Why insightful:** The issue states A_h=m·conj(τ_h)·T_h and N₀=Ση^r/q separately; chaining them shows the COUNT N₀ is literally a self-correlation moment of unit Gauss phases restricted to the character-relation lattice — the entire difficulty is phase coherence of {u_χ}, amplitude provably inert.

**Attack:** Probe _wf_gaussphase_relation_moment.py: compute u_χ=τ(χ)/√q for χ∈μ_n^⊥, form M_r=Σ_{χ_1···χ_r=1}u_{χ_1}...u_{χ_r}, check N₀(G,r)·m^r/q^{r/2}=M_r exactly; test whether M_r/(diagonal Hasse-Davenport count) stays O(1) to depth r≈log q (the Betti/Katz-relation question). Lean: rawMoment_eq_gaussPhase_relation_sum brick over SubgroupGaussSumMoment + gaussSum norm.
...
**Why insightful:** Reframes W-Johnson not as a frequency/energy deficit but as a base-point union tax, and pins the exact factor (q^{n/2}, dwarfing the budget) with machine-checked two-sided bricks. Shows why ANY L² estimate is hopeless and exactly what an L^∞ (every-s0) argument must save.

**Attack:** Probe whether the worst s0 (achieving ‖𝒮‖ near √(|V|·M)) is STRUCTURED — does it lie in a coset of H^perp? If forced into a low-dimensional structured set, the union tax shrinks from √|V| to √(dim). Lean: characterize argmax_{s0}‖shawError S s0 s1‖ via the phase Σ_ψ 1̂_S(ψ)ψ(s0) being a flat exponential sum in s0 (Salem-Zygmund flatness, F16), connecting the base-point sup to F16.
...
**Why insightful:** The issue cites BCDZ25 Thm1.11 vacuity (d(k−d)/(s−d+1)) as external folklore; this shows the SAME wall is already a machine-checked dichotomy in-tree (proven τ-lower meets proven off-range τ=1), with the Schubert denominator realized as the [s]-range cutoff. The wall is not a missing lemma — it is two proven theorems whose ranges fail to overlap at s=1.

**Attack:** Lean: subspaceDesign_listsize_vacuous_at_s_one — for s=1 and any r≥2, the hypothesis τ r·n+r·n<(r+1)·a implies a>n (impossible since a≤n), using τ(r)=1 from the FRS profile. Makes the vacuity a theorem. Probe: tabulate (s−d+1) vs the [s]-range cutoff to confirm they coincide as the design-quality denominator.
...
**Why insightful:** Identifies the orbit-law multiplier c (an opaque field element g^{b−a} in the proof) with the homogeneity degree of the complete-homogeneous symmetric polynomial governing the residual — a structural reason for the n/gcd orbit sizes, bridging F15 (Schur/h_j vanishing) and F9 (orbit count) by a degree=eigenweight identity.

**Attack:** Lean: generalize residual_ladder_schur (b=k+1) to arbitrary b≥k+1: residual dom k t (x^b)=h_{b−k}(dom(t))·residual(x^k) via Jacobi-Trudi reduction of X^b mod the core-vanishing polynomial. Then prove h_{b−k}(g·x_S)=g^{b−k}·h_{b−k}(x_S) to derive the orbit multiplier abstractly. Probe (probe_ratio_collision_census.py has k=2/k=3 Schur data): extend to b=k+2 to read off h₂ and confirm the g²-orbit.
...
**Why insightful:** Contradicts the comforting framing that F10 (count) is independent of F2/F5. The Newton/power-sum=syndrome identity shows the lacunary variety nonemptiness for LARGE t is governed by the same antipodal-balance/E_r structure whose char-p transfer is the wall; q-independent only while t<2·r_max≈2(β+1)log_n p.

**Attack:** Lean: instantiate MvPolynomial.NewtonIdentities to prove vanishingVariety (e-window)=power-sum-window variety over CharZero, then connect to EnergyRelationAntipodal showing variety members are antipodally balanced. Probe: measure at what gap t the char-0 coset-union count and the char-p count first DIVERGE — predict t*≈2(β+1)log_n p.
...
**Why insightful:** The campaign treats Action-Orbit (F9, comment 15) and DyadicLacunaryFloor (F10) as DISTINCT escape lanes; recognizing they are the same ⟨g^{|b-a|}⟩-coset action makes K and #lacBad mutually computable and routes Chai-Fan's open Q1 into the in-tree esymm-vanishing (F15) framework with the Vieta pin already proven.

**Attack:** Lean: lacBad_card_eq_orbitCount_mul_cosetSize equating DyadicLacunary.lacBad cardinality with valueSpectrum_card_le_of_orbit_param (OrbitSpectrumBound) at g^t. Probe _wf_orbit_eq_lacbad.py: at proper subgroup compute Chai-Fan K (orbit count of bad-α) and #lacBad/(n/gcd(t,n)); verify equality across rates and directions, translate Chai-Fan's F_d(α) norm to the e_t-vanishing count.
...
**Why insightful:** Two already-proven in-tree bricks combine into a clean upper bound on the deep-moment ANOMALY (the object comment 14 measured exploding), reducing the wall to a pure point-count in the 2r-box — separating the proven L^∞/L² caps from the one genuinely open equidistribution count.

**Attack:** Lean: energy_anomaly_le_autocorr_times_latticeCount combining autocorr_le_autocorr_zero with no_spurious_tuple_of_lt_prime to bound E_r^{Fp}−E_r^{char0}≤R_r(0)·#{contributing 𝔭-cosets}. Probe _wf_anomaly_decomp.py: verify the anomaly equals Σ_{z≠0,p|z}R_r(z), check R_r(z)/R_r(0)≤1 always, measure the contributing-point count vs the (2r)^{φ(n)}/p box-density prediction to localize the open Bourgain-Shkredov factor.
...
**Why insightful:** Sharpens the open core from 'bound |η_b|' to 'magnitudes are already pinned by proven moments; only the phase alignment (F16 Salem-Zygmund) is open' — a strictly smaller and more falsifiable target, grounded in two landed two-sided level-set bricks. Cleanly separates what the moment method gives (magnitude) from what it cannot (phase).

**Attack:** Probe: at μ_n over p≈n^4, fix the proven magnitude profile {‖η_b‖²} and ask whether ANY phase assignment consistent with the Gauss-sum constraints (|τ(χ)|=√p, Hasse-Davenport only, F19) could push max|η_b| above C√(n log m). If phase constraints FORCE flatness, that is the closure direction; if they admit a coherent worst case, that bounds δ* from above. Lean: state PaleyFloorBound as 'magnitude-pinned + phase-flat', prove the magnitude half from card_resonant_ge/prize_levelset_sharp.
...
**Why insightful:** Comment 1's 'missing reverse bound' and the design route's 'list-dim confinement' have never been identified as the same object. The design machinery is PROVEN (axiom-clean) for the folded case; recognizing it as the reverse bound the floor needs tells you exactly what folding is buying analytically (the rank cap).

**Attack:** Instantiate subspaceDesign_list_dim_bound at the monomial-line family: close codewords={u0+γu1 δ-close}, their differences ∝γ·u1 are 1-dimensional so r=1 and the bound is |L|≤1 unless τ(1)·n is large. Compute the τ(1) needed to match the n² Poisson count; this reveals the exact τ-deficit plain RS suffers. If the deficit equals H(ρ)/(β log n), the design wall and the floor coincide quantitatively.
...
**Why insightful:** Reframes the dead 'push past r=2' AG plan: the obstruction (Betti grows in r) is precisely the parameter Stepanov does NOT need to grow. Stepanov fixes r and pushes M — so the Betti wall that kills the AG route is not a wall for Stepanov. The campaign filed Stepanov under 'ruled out' but that referred to the GENERIC dimension count; the special-form A-degeneracy auxiliary is a different live object.

**Attack:** Build the order-3 confluent auxiliary by analogy to Q: seek Ψ vanishing to order 3 at every y∈A=μ_n∩(c+μ_n) with deg Ψ≈n, giving r(c)≤(n+1)/3. Lean: repCount_three_mul_le : 3·r(c)≤deg Ψ≤n+O(1). If the order-M family exists with deg Ψ=O(n+M²), optimizing M gives r(c)≤O(n^{2/3}), E_p≤O(n^{8/3}) — a FIXED-r-free char-p energy bound the AG route cannot produce. Probe first: numerically search for the order-3 auxiliary at μ_8/F_p.
...
**Why insightful:** Quantifies the marginal-method ceiling exactly: a per-period bound costs q^{1/4}; only the joint max-of-m removes it. Makes 'decorrelation' concrete as the q^{1/4}→√(log m) improvement, connecting the proven Sidon energy bound to why no single-frequency estimate suffices.

**Attack:** Lean: sidon_perPeriod_vs_extremeValue_gap stating (3q)^{1/4}√n exceeds √(2n log m) by q^{1/4}/√(log m), framing the missing piece as chernoff_max_re_le. Probe: at proper subgroups compare (3q)^{1/4}√n against measured max_b‖η_b‖ to show the latter is √(log m)-scale, not q^{1/4}-scale.
...
**Why insightful:** The spectral identity and the coset count exist separately; fusing them shows the line-incidence error inherits the coset multiplicity-n structure, so F3's worst case is a max over m periods — explaining why the budget n·log(p/n) (=variance n × max-of-m enhancement) is the right scale, bridging F3 to F7/F18 not just F2.

**Attack:** Lean: lineIncidence_error_eq_period_average rewriting the s₁^⊥ spectral sum using eta_constant_on_mulCoset to collapse to Σ over m period values × multiplicity n. Probe: verify #bad far-line incidence(δ)=avg-of-n·(period values in the syndrome ball) exactly at a proper subgroup, and that its variance = F18 autocorrelation max.
...
**Why insightful:** Three files describe the breakdown with three objects (cyclotomic resultant, additive energy, Jacobi average) and none cites the others; identifying (2r)^{φ(n)}=p as the COMMON crossover unifies the algebraic, combinatorial, and multiplicative descriptions, reframing 'why the moment route walls at r≈β' as 'when the first signed root-of-unity relation fits in the (2r)-box mod p'.

**Attack:** Probe: for μ_n at large prime, correlate (a) smallest r with E_r>E_r^{(0)}, (b) smallest r with max_h|T_h| exceeding char-0 Jacobi value, (c) r where (2r)^{φ(n)} first exceeds p — verify coincidence. Lean: tangent_defect_iff_spurious_tuple stating T_h deviates from char-0 iff a spurious tuple exists mod p, connecting no_spurious_tuple_of_lt_prime to a tangent-sum clean range (hence to B via I3).
...
**Why insightful:** KU vacuity is read as 'no tool reaches the prize'; cross-referencing with the chaining route (one MGF) and the Jacobi identity shows the needed object is weaker and on a different surface — redirecting effort off the dead Wasserstein/BGK lane onto F19.

**Attack:** Lean: state EffectiveJacobiAverageMGF (the empirical MGF of Re(ζ̄·T_h) over h∈[m] is ≤exp(σ²λ²/2), σ²=O(p/m)) and prove it IMPLIES SalemZygmundChaining.SubGaussianMGF via I3 (τ_h flat). Probe: measure the Jacobi-average tail constant (proper subgroups, multiple primes) to check it stays O(1) where the Wasserstein bound is vacuous.
...
**Why insightful:** The campaign treats the norm threshold (clean range) and the forced-anomaly crossover as separate facts (one geometric, one Fourier-positivity). They are the same number r*=½p^{1/φ(n)} read off the single norm functional; the extremal α=c·ζ^e is simultaneously the norm-maximizer AND the crossover-locator.

**Attack:** Probe: r*_norm=½p^{1/φ(n)} vs in-tree r*_anomaly (smallest r with q·E_r^{char0}<n^{2r}) across n=8..64, β∈{4,5}, multiple primes; verify coincidence within the 2-adic-multiplicity correction. Lean: crossover_eq_norm_threshold relating no_spurious_tuple_of_lt_prime's (2r)^{totient}<p to the forced-anomaly inequality, making the two crossovers ONE definition.
...
**Why insightful:** Converts the verbal 'structural root of the wall' into a one-line congruence theorem (q≡1 mod 2^μ) that is the SHARED hypothesis of the prize regime and the open ideal-SVP class, fully Mathlib-formalizable, giving a citable obstruction-explanation.

**Attack:** Lean (achievable now): smoothDomain_iff_fullySplit : (∃ζ:ZMod q, IsPrimitiveRoot ζ (2^μ)) ↔ (q%(2^μ)=1), tied to splitting of cyclotomic (2^μ) over ZMod q via isRoot_cyclotomic_iff. Machine-checked Prop. Probe: enumerate primes q∈[n^4,n^5] with/without order-2^μ subgroup and check residue mod 2^μ.
...
**Why insightful:** The issue treats the additive 2-adic tower (F8) and the cyclotomic norm threshold (W-anomaly) as separate lanes; recognizing both as the same φ(n)-fold product, with the tower as the 2-adic stratification of the norm, is unstated structural unification.

**Attack:** Probe: for short balanced 2r-tuples α=g(ζ_n) at n=16,32, compute per-embedding |g(ω)|, check clustering by 2-adic tower level (predict small factors concentrate where (1+w)/c has order exactly 2^{j+1}, per GVObjectTowerDecomposition). Lean: factor Res(Φ_n,g) through the tower factor product of TwoPowerTowerFactorization for n=2^μ. If one level dominates the smallness, the defect-existence wall localizes to a single quadratic factor.
...
**Why insightful:** Diagnoses the exact reason the additive tower-martingale loses √(2 ln m) (bulk-vs-tail) and pairs it with the in-tree observation that cancellation is multiplicative/odd-order, proposing the Jacobi-sum cocycle as the loss-free replacement — a connection between F8 (failed) and F6/F19 (the right surface).

**Attack:** Probe: compare the additive level cocycle (period ratios r_j) with the multiplicative Jacobi cocycle (track |T(φ)| as φ ranges and as the field lifts F_q→F_{q^2}); test whether the Jacobi-tower has bounded per-level multiplier. Lean: combine tangentSum_mul_orderOf_eq_sum_jacobiSum with a Hasse–Davenport lift lemma to express T over F_{q^2} as a product over the quadratic descent, exhibiting the multiplicative cocycle; check the increment is uniformly bounded.
...
**Why insightful:** Unifies the count face (F4, the proven full_tower budget) and the house face (F2, the open halving) as one s-indexed squaring-tower recursion, showing the list-size budget 2^{n/2^s} is the SAME object as the per-level house multiplier at a different window depth.

**Attack:** Probe: for fixed q and μ_n, compute both the actual bad-scalar count at window t=2^s−1 and the house M at fold depth s, s=1..μ; test the 2^{n/2^s} branching prediction. Lean: exhibit DyadicHalvingRecursion.eta_halving and LamLeungTwoPow.full_tower as instances of one squaringFold operator at s=1 and s=μ, with window depth s as parameter.
...
**Why insightful:** Couples the proven reality (F18 root) with the parallelogram to show the descent is a real 2D rotation, identifying ‖A−B‖=the odd-half period as the exact quantity whose non-smallness breaks coherence — tying the gap to F10 antipodal rigidity, a link the issue does not make.

**Attack:** Probe: at worst coset b*, measure ‖A−B‖²/M(k)² and ‖A‖²+‖B‖² across the tower; test whether ‖A−B‖² stays Ω(M(k)²) (forcing ‖A‖²+‖B‖²<2M(k)²). Lean: the real parallelogram via conj_eta, twistedPeriod=A−B, Prop TwistedPeriodLowerBound whose truth yields soft descent; relate to LamLeungTwoPow.vanishing_sum_antipodal.
...
**Why insightful:** Lifts the n-divisibility from the special monomial ladder to the full eigenstack class via an already-proven general law, and ties it to the EsymmFiber characterization of sub-Johnson cores — the orbit quantization is a property of the whole F4 list-size object, not a monomial artifact; the KKH26 ceiling line and toy-plateau maximizers are unified as one quantized object.

**Attack:** Lean: instantiate orderOf_dvd_badScalarSet_card_of_eigenstack at the KKH26 near-capacity eigenstack (eigenratio g^{−m}, order s) to get s∣#bad, cross with EsymmFiber count to show the near-capacity list is s-quantized; verify against the in-tree N(4,4)=1233=77·16+1 census. Probe (probe_s3_eigenstack_orbit_law.py validates the (13,12,6) profile): extend to a twisted (non-adjacent) eigenstack.
...
**Why insightful:** Exhibits the same domain with two subset-operations giving provably-opposite spectrum sizes (n vs open/super-exp), isolating that the prize hardness is purely additive (multiplicative side trivially tame), and names the dilation orbit as the common symmetry.

**Attack:** Probe probe_mult_spectrum.py vs probe_jump_subsetsum.py side-by-side on μ_8..μ_64: confirm product spectrum=n exactly while sum spectrum grows; measure sum-spectrum/n=K to test polynomial-in-n vs super-poly. Lean: formalize that g·(subset-sum) is a subset sum (exponent shift) to give the Z/n action, then sum-spectrum-card ≡[0 realized] (mod n).
...
**Why insightful:** Both refutations ASSUME kurtosis 3 to manufacture the far atom, yet the in-tree moments PROVE kurtosis <3 strictly. A one-sided (μ_4≤3−c) extremal problem is a genuinely different LP than the two-sided one solved, and nobody has run it.

**Attack:** Run the Chebyshev-Markov extremal LP for the largest support atom of a symmetric mean-0 measure on [−√n,√n] subject to μ_2=1 AND μ_4≤3−3/n (inequality), at m atoms. Compare to the equality-μ_4=3 bound. If the strict-below + compact support pulls the bound below n^{1−ε} at fixed R, formalize the LP-dual certificate. Probe first: extend probe_407_markovkrein_sharp.py to impose μ_4 as an upper-bound constraint, sweep the gap c=3/n.
...
**Why insightful:** The cone has the odd-moment law landed and explicitly marked 'unaffected by refutations', yet it is used NOWHERE in any B-bound — every route symmetrizes first. Re-coupling the proven odd moments into the extremal problem is an untried tightening using already-proven char-0 math.

**Attack:** Add the odd-moment constraints Σηᵢ^{2k+1}=−n^{2k} (k=1..R) to the Chebyshev-Markov LP for max-atom and re-solve; the extra equality constraints can only shrink the feasible far-atom. Probe: python LP/SDP with both even (Bessel) and odd (−n^{2k}) moment rows up to R=ln m, report whether the joint-constrained max-atom drops below n^{1−ε}. If yes isolate the dual certificate; if odd constraints inactive at the optimum, log honestly.
...
**Why insightful:** The cone repeatedly notes 'B is the Paley second eigenvalue' and 'the difficulty is bulk extreme-value not outliers' — EXACTLY the regime where free-probability edge estimates (Haagerup-Thorbjørnsen, BBP) apply — yet no route uses the free-cumulant/R-transform edge. The classical-vs-free cumulant distinction (κ_4^{cl}=−3/n vs κ_4^{free}=1−3/n) is a genuinely new lens.

**Attack:** Compute the free cumulants from proven μ_2,μ_4,μ_6=15−45/n+40/n² (moment→free-cumulant Möbius on noncrossing partitions) and the implied BBP/edge prediction; compare to measured B. Probe: free-cumulant calculator on the in-tree exact moments, test whether the edge tracks B/√(n ln m). If free cumulants O(1) bounded, edge is √n·polylog — formalize the free-cumulant→edge bound as the named open input.
...
**Why insightful:** Exhibits a reverse bound that DOES match across faces (F4↔F2↔F5), unlike the upper bounds (which only the energy face controls, capped at Johnson). The shared origin (antipodal zero-sum subsets = diagonal energy) explains WHY the reverse bound is uniform while the forward bound is not — a structural asymmetry not noted in the issue.

**Attack:** Lean: supply_lb_eq_period_lb_at_diagonal showing both reduce to C(R.card,j) via the raw-moment identity Σ_b η_b^{2r}=q·E_r. Clarifies that pushing the reverse bound past √n requires NON-diagonal (off-antipodal) words on BOTH faces simultaneously.
...
**Why insightful:** The collision-spectral and Gauss-sum-second-moment files were written for different issues and never linked; recognizing they are the SAME Plancherel at two group levels lets plancherel_collision_main_term transfer verbatim to the η-moment ladder, and identifies N₀ as a genuine subset-collision count (not just a phase sum), opening Paley-Zygmund anti-concentration on the η side.

**Attack:** Lean: instantiate plancherel_collision with stat S=Σ_{x∈S}x over A=F, a=r, prove it equals Σ_b‖η_b‖^{2r}/q=E_r (matching r=1), giving eta_moment_eq_collision. Port SubsetSumPaleyZygmundDichotomy.manyHeavyTargets_card_ge to the η-frequency side: 'small η-collision second moment ⟹ many heavy frequencies', a new anti-concentration constraint on the Gauss-period distribution attacking F2.
...
**Why insightful:** Most attacks chase a spectral-gap (Ramanujan) upper bound; the proven coset-count + second-moment variance explain WHY that is the wrong object (m distinct eigenvalues force a √log m excess by EVT) and why the constant is √2 not 2. The exact #values·|G|≤q−1 makes 'log((q-1)/n) not log q' a theorem.

**Attack:** Lean: combine eta_image_card_mul_le (≤m values) with subgroup_gaussSum_secondMoment (variance n) into paleyFloor_from_evt: IF the m periods satisfy a sub-Gaussian tail with proxy n (the SalemZygmundChaining input) THEN PaleyFloorBound ψ G (√2)(log m). Probe: verify B/√(2n log m)→1 and B/(2√n)→∞ across growing m at fixed n.
...
**Why insightful:** A precise non-overlap theorem: the one face with no BGK wall (F12) is exactly orthogonal to the negation-symmetric locus that all energy/Bessel/Lam–Leung machinery describes — so importing those tools is provably futile, and the correct tools are dilation-orbit counting (F9) and quadric geometry, redirecting effort away from a dead transfer.

**Attack:** Lean: land badLocus_disjoint_negClosed (formalize e2_zero_bad_not_neg_closed from e1_eq_zero_of_neg_closed). Then parametrize {e2=0, e1≠0} by the complement-quadric and dilation orbits, prove the residual-orbit count K is O(1) above δ* via the quadric's bounded point count — the only route consistent with this disjointness.
...
**Why insightful:** Flips the hard high-agreement rows (a close to n) into low-dimension (c small) quadric geometry where the point count is governed by classical quadratic-form theory over F_q — a tractable handle, the algebraic dual of the half-sum lemma.

**Attack:** Lean/probe: for c=2,3 write the quadric S_T²+Q_T explicitly, count μ_n-points via the classical quadratic-form discriminant (Mathlib quadratic forms / Gauss-sum-of-quadratic). The a=n−3 row is probe-reported fully coset-decomposable; formalize its point count = the orbit census directly, giving an exact closed δ* value on the top rows independent of BGK.
...
**Why insightful:** Identifies a uniform method (count = invariant of a finite group action) producing hard q-independent constraints on BOTH the algebraic F12 face and the analytic F5 kernel without touching the magnitude — the right object is the count MODULO the full symmetry group (residual orbits), reducing both faces to the same residual-orbit-count question.

**Attack:** Lean: a common count_dvd_of_group_action lemma, rederive both three_dvd_bgkCount and a divisibility refinement of the e2 bad set. Research: enumerate the full symmetry group acting jointly on {e2=0, e1≠0} subsets (negation, dilation by ⟨g^t⟩, Frobenius x↦x^p) and compute the residual orbit count K — if the combined group forces K=O(1) above δ* by pure orbit arithmetic, that is a non-analytic closure of the depth-1 floor.
...
**Why insightful:** Reframes the lane not as 'index too big' but as 'we throw away cancellation among the m phases by triangle-inequality'; the proven QR closed form is literally the J=1 collapse, suggesting a graded family of partial-collapse bounds between QR (clean) and bare √q (trivial) parametrised by how many phases you can pin.

**Attack:** Probe _407_consthead_split.py: for fixed small q and μ_n compute partial sums S_J=‖Σ_{j<J}τ(χ^{dj},ψ_b)‖/(J·√q) vs full B; measure whether a constant/polylog head J already captures the bulk (S_J/B→1 fast) or all m phases are needed. If a polylog head suffices, target a Lean lemma bounding the head via gaussSum norm + explicit phase-pinning for χ^{dj}(b) at j in an arithmetic progression.
...
**Why insightful:** Links the proven parity fact to the constant-index lane's recursion barrier: people assume the dyadic structure powering F8 could recurse the phase DFT, but odd m forbids it — F8 and F20 live on coprime-order sides of F_q*≅μ_n×ℤ/m.

**Attack:** Lean: constIndex_no_dyadic_recursion combining prizeIndex_odd + doubling_bijective_of_odd to certify that any sub-√q saving in the m-phase completion sum must come from cancellation over an ODD-order cyclic group (no 2-adic tower), i.e. from Salem–Zygmund flatness over ℤ/m (F16), NOT F8. Documents why the 'fold the index by 2' attack is dead.
...
**Why insightful:** Unifies the energy floor's hypothesis with the e₂=0 face and the all-witness fit face — one char-p coincidence count. The §5.7 e₂=0 'no BGK wall' claim is in tension: the SidonModNeg/energy floor IS gated on the same coincidences, so e₂=0's freedom from BGK must come from the resultant-height threshold, not from avoiding the coincidence count.

**Attack:** Compute, for β≈4 and μ_n, whether SidonModNeg holds = whether extra additive coincidences appear, cross-tabulate against the all-witness fit-count and the e₂=0 rigidity threshold. Lean/probe: a single Prop NoShortVanishingRelation μ_n p, show it implies SidonModNeg, the all-witness sharpness, AND e₂=0 rigidity — if provable all three close together; the honest residual is then this one Prop = the resultant-height/Lam–Leung char-p transfer.
...
**Why insightful:** Gives an exact dictionary 'agreement-spectrum binomial moment ↔ additive deep moment', so the deep-moment-anomaly wall (W-anomaly) has a combinatorial twin: the supply moment Σ_c C(a_c,t) is forced above its char-0 value by the same zero-sum coincidences. The two 'moment walls' are one.

**Attack:** Cross-check whether the supply-moment crossover (where Σ_c C(a_c,t) exceeds the frozen-base extrapolation) lands at the same t*≈β+1 as the W-anomaly r*. Probe: for fixed μ_n and a sweep of capped words, compute Σ_c C(a_c,t) vs C(n,t)·(scaling), locate the spike t; compare to E_r anomaly crossover. If coincident, the supply face inherits the anomaly and the two walls are provably identical.
...
**Why insightful:** Identifies a single algebraic gadget (distinctness-preserving surjection) underlying three in-tree results — Mersenne covering descent, curve interleaving (Jo26 5.7), the cyclotomic lift to F_p — and shows the fully-split prime 𝔭|p (the Pan-Xu open case, F11) is the relevant hom for the prize.

**Attack:** Lean: an abstract DistinctnessPreservingTransfer generalizing sumsetDistinct_image_eq_univ to the curve-close-set image, instantiate with the cyclotomic reduction ℤ[ζ]↠F_p (degree-1 𝔭). The injectivity hypothesis becomes p∤Res(f_S−f_T,Φ_{2^μ}) (not_dvd_collisionResultant). Probe: for fully-split prize primes, measure the density of degree-1 primes 𝔭|p for which the dyadic sumset stays injective vs collapses (the F11/Pan-Xu split-case count).
...
**Why insightful:** Connects the geometric collision matroid (dual pencil / (e,m) plane) to the orbit-quantization and the F15 Schur-vanishing face — three faces of #bad in separate files, not linked. Shows the open 'slanted Dickson family' is exactly the non-orbit, non-antipodal residual the prize needs to count.

**Attack:** Lean: bridge MCASecondMoment's collCount/c_T to dualVec (show c_T=⟨λ^T,·⟩ on the deep-hole word) so dependent_iff_collinear directly counts functional collisions; then bad-scalar count = #distinct c_T = M'−(#collinear-(e,m)-triples). Probe: enumerate slanted (cross-class collinear) μ_n triples — sections of m=ζ^s ⟹ e=ζ^j+ζ^{s-j} (Dickson) — at μ_16 to get the exact non-structured count.
...
**Why insightful:** Non-obvious that the proven low-order energy LOWER bound (a W-Johnson liability) and the high-order cumulant UPPER margin (the prize asset) are two faces of one constraint (compact support). Says the energyExcess and the cumulant are not independent: a bound at r=2 should propagate to κ_r at all r via the shared support — the missing link from proven r=2 to open r≈ln q.

**Attack:** Test propagation: does the proven E_2=3n²−3n (support [−√n,√n]) algebraically FORCE κ_r≤1 for all r via a support/moment interpolation (Hausdorff-moment / completely-monotone on the compactly-supported measure)? Probe: given exact μ_2,μ_4 and support cap, compute the maximal feasible μ_{2r} (Markov upper) vs the Bessel baseline (2r−1)!!n^r; if support+μ_4 already keeps μ_{2r} below baseline at all r, the open core collapses to proven r=2. Lean: 'compact support √n + μ_4≤3 ⟹ μ_{2r}≤(2r−1)!!n^r' if the LP confirms.
...
**Why insightful:** Unifies two faces worked SEPARATELY (#232 collision/moment scalar and #389 Shaw line-incidence) as a single Plancherel identity on different dual groups, showing the deep-moment wall and the line-incidence wall are not two walls but one off-diagonal spectral mass.

**Attack:** Lean: a meta-lemma abstracting both plancherel_collision and shawError_second_moment as instances of one plancherel_energy(stat)=Σ_ψ‖charSum stat ψ‖² over an arbitrary finite abelian target; specialize to recover both. Lets any future bound on Σ_{ψ≠0}‖Tψ‖² feed BOTH the F5 and F3 consumers. Verify collision(μ_n,a,sum-stat)·q=(C(n,a))²+Σ_{b≠0}|elementary-symmetric-in-η|² at small n.
...
**Why insightful:** The explicit constant the literature uses without writing down, making the F18 autocorrelation face a literal corollary of the F5 energy face via the dilation reduction — that T(H) (curve incidence, the Szemerédi-Trotter/Stepanov object) equals the autocorrelation sum is not in the issue body.

**Attack:** The honest residual is the n^{1/2} deficit (E≥n² always, √(nE)≥n^{3/2}>budget n). Attack T(H) directly via the dilation-reduced curve: bound #{(b,c)∈H²:1+b−c∈H} by a Stepanov auxiliary-polynomial on the curve 1+b−c=h∈H, exploiting that |H|∣E removes one DOF. Lean: a T_le lemma feeding addEnergy_eq_card_mul_incidence — even T(H)≤n²·polylog/q gives the F2 √(n·polylog) target via the transport.
...
**Why insightful:** Re-identifies which automorphism Rojas-León's √q-gain should act on: not the additive x-sum (needs n≥√p, empty) but the multiplicative h-shift of the Hasse-Davenport-linked Jacobi family (m statistics, available). The 'wrong uniformity' the issue flags becomes 'uniform-in-p at fixed index m' — a precisely stated, possibly-tractable effective conductor question.

**Attack:** State the conductor of the h↦T_h family (Kummer sheaf L_{χ^h} twisted by the tangent locus) and check whether it is bounded uniformly in p at fixed m — if yes, effective Deligne gives √-cancellation in the m-average. Lean (modest): wire EffectiveTransfer's threshold to the tangent-sum vanishing, tangentSum_reduction_ne_zero: a nonzero char-0 tangent value survives mod p above C(n,·)^{φ(n)}, pinning per-prime exactness of T_h analogous to esymm_eq_zero_iff.
...
**Why insightful:** The L1/triangle route was refuted because it drops h-cancellation; the Gauss-quotient form reveals (unit_h·T_h) IS an autocorrelation, so its b-DFT is a power spectrum (nonneg, Parseval-tight). The algebraic reason the house can't be bounded by L1 but CAN by a spectral/positivity argument — entirely Mathlib+in-tree, no Weil-on-curves.

**Attack:** Lean: house_is_power_spectrum — from I3+I4 and subgroup_gaussSum_moment at r=1, show |η_b|²·m²=|Σ_{j<m}χ^{-j}(b)τ_j|² is the manifest nonneg spectral form, confirming no L1 bound is tight and isolating the positivity structure. Probe: verify Σ_h unit_h T_h χ^h(b) reconstructs m²|η_b|²/p−const exactly (1e-12) at proper subgroups.
...
**Why insightful:** The negation symmetry has only been used additively (S_b∈ℝ). 407-TANGENT shows it acts on the MULTIPLICATIVE tangent geometry too (w↦w⁻¹ involution). If this halves the Betti number in the GLT Fermat-point-count, it pushes the W-Betti crossover from r*≈3 toward r*≈3+ — small but a structural degree reduction not previously exploited on the tangent face.

**Attack:** Probe: compute the cohomological/Betti dimension of Σ_{w∈μ_n}χ^h(1−w) as a sum over μ_n/{±1} (over squares t=w²) and compare the effective curve degree to n vs n/2; check whether the GLT r=2 Hasse-Weil result extends to r=3 on the halved curve. Lean: tangentSum_involution: T_h=Σ_{orbits}(χ^h(1−w)+χ^h(−w⁻¹(1−w))), exposing the χ^h(−w⁻¹) factorization.
...
**Why insightful:** Every comment uses only dilation/permutation/translation symmetries; the twisted-inversion (GRS duality) monomial automorphism is in-tree, proven, and ignored on the prize cone. It is the F2↔F3 self-duality (period b↦b⁻¹ vs scalar γ↦γ⁻¹), a second commuting symmetry no published incidence bound exploits.

**Attack:** Probe: extend the FarLineIncidenceEquivariance worst-line sweep to quotient the n² monomial directions by ⟨dilation, twisted-inversion⟩ and check the worst direction is a FIXED point of the involution (self-dual line a+b=k). If forced self-dual, the circulant line-incidence operator (F3) restricted to the inversion-symmetric subspace is a smaller real symmetric problem. Lean: package explainableScalars invariance under the inversion permutation to land the combined ⟨Z/n,inv⟩ equivariance.
...
**Why insightful:** Two independently-built pieces — the conditional Plancherel bridge (per-character suffices) and the Stepanov engine (one curve, non-vanishing free) — are exactly complementary: the bridge needs per-ψ bounds, Stepanov produces per-curve bounds. The campaign treats the Weil input as monolithic; this exposes that for the M2/second-moment face the input is per-character (Johnson-level, F18 flatness), where the assembled Stepanov machinery has a real shot.

**Attack:** Wire collision_le_of_relative_bound's hB to weil_stepanov_card_lt's conclusion: instantiate (g,A0,A1,V,M) so V=the curve {stat(x)=t}∩μ_n and |V|·M<D yields ‖Tψ‖²≤ε·C². The remaining piece is choosing A≈q/2−deg g and M≈√q to balance 2ℓ(A+1) vs |V|·M — pure degree bookkeeping. Lean: perChar_weil_bound_of_stepanov_aux discharging hB from an explicit auxiliary, then chain to M2 anti-concentration — closes the SECOND-MOMENT (Johnson-side) face unconditionally even if deep-band δ* stays open.
...
**Why insightful:** Reframes the open core as a Lyapunov-exponent/large-deviation statement (limsup running geometric mean), strictly weaker than the in-tree refuted per-level descent and distinct from the just-refuted Azuma sum-of-L∞ — a distinct, untried inequality shape.

**Attack:** Probe: compute r_j across the full tower j=1..μ for several large primes, plot running geometric mean ∏_{i≤j}r_i^{1/j}; test convergence to √2 from above despite per-level spikes (predict yes, matching mean-2.1). Lean: towerLyapunov=(1/μ)·Σ log(M(2^j)²/M(2^{j-1})²), state the live conjecture towerLyapunov→log 2 with the strict-descent refutation recorded as why per-level fails.
...
**Why insightful:** A proven axiom-clean closure of the count face on a full infinite prime family (n|p+1) the KB does not foreground for #407. Reframes the prize as 'the split-prime surplus over the inert-prime baseline r≤2' — a sharply localized arithmetic difference, suggesting a deformation/interpolation between the two prime families.

**Attack:** Probe: for fixed n=2^μ, compute worst-case I(δ) and #lacBad over BOTH inert (n|p+1) and split (n|p−1) prime sequences; confirm inert stays O(n) (r≤2), split grows the Θ(n⁴/p) surplus. Investigate whether a CRT/quadratic-extension lift transfers the inert r≤2 to control the split count at frequency b in the F_{p²}-coset (Frobenius-pairing the split sum with an inert one).
...
**Why insightful:** The Azuma no-go is explicitly attributed to 'Azuma can only use the L^∞ bound'; the Bessel law provides the full increment LAW (hence its MGF/conditional variance), the ingredient distinguishing Bennett (MGF) from Azuma (range). Coupling the proven Bessel MGF to the proven uncorrelatedness has not been tried.

**Attack:** Compute the Bessel-law MGF of a single period increment from I₀(2√x)^{n/2}, feed into a Bennett/Bernstein tail for the telescoped η^{(2^a)}=ΣΔ_k with ⟨S⟩=Σ Var(Δ_k)≍n. Check whether it beats the √(2 ln m) Azuma inflation. Probe: extend _cumulant_martingale_deep.py to compute the empirical conditional MGF of Δ_k and test B_a≤√(2⟨S⟩ ln m)+(1/3)b_max ln m vs target — quantify whether the Bessel-controlled variance term dominates the b_max ln m obstruction.
